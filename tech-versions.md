...
| レッスン | モデル | 役割 |
|---------|--------|------|
| 全体像（Day 28 / 旧 drafts/108） | 両方 | 4 種類のキャッシュ＋2 モデルの俯瞰。シリーズの入口（公開済み） |
| データキャッシュ（drafts/105） | 従来 | `fetch` オプションでのデータキャッシュ |
| Full Route Cache（drafts/106） | 従来 | 静的/動的と HTML のキャッシュ |
| Router Cache（drafts/107） | 共通 | ブラウザ側の保持。新旧で同じ |
| Request Memoization（drafts/104） | 共通 | 1 リクエスト内の重複排除。新旧で同じ |
| use cache でキャッシュする（drafts/038・改修予定） | 新 | `"use cache"` / `cacheLife` / `cacheTag`。従来の 105＋106 に相当 |

- 107・104 は新旧共通なので、新モデル版は作らない
- 新モデルの Suspense は既存の「Suspense と Streaming」（drafts/039）が受け皿。新規レッスンは作らない
- 新モデルで新規に要るのは 038 の改修 1 本のみ（105＋106 を `"use cache"` 1 本に統合する位置づけ）
- `fetch` 以外の取得例は、重複排除（104）では Redis（セッション読み取り）を使う。永続キャッシュ（105・038）で Redis を使うと「キャッシュをキャッシュする」不自然な例になるため使わない
