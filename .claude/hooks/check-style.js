...
//   7) 本文の地の文が1行に「。」3つ以上（3文以上）詰め込まれている（短い段落・箇条書きに分解する）
...
const SCAFFOLD = [
  "結論から言うと", "まず全体像", "を見ていきます", "という地図", "大きく2つ", "大きく3つ",
  // ドラマチックな前置き・教科書的な締め（2026-07-20 のディスレクシア回で目視をすり抜けた型）
  "仮定の話ではありません", "と言えるでしょう", "という設計です", "だということです",
];
...
function scan(file) {
  ...
  lines.forEach((line, i) => {
    ...
    // 数え上げの宣言（「仕事は 2 つで」「違いは 3 つです」）。予告せず中身から書く。
    // 「AとBの 2 つで対処できる」のような後方参照は正当なので、候補として人が判断する。
    if (/[はがも]\s?(大きく)?\s?[0-9０-９一二三四五六七八九十]+\s?つ(です|で[、す])/.test(noCode)) {
      hits.push(`  [数え上げ宣言の疑い] ${i + 1}: ${t.slice(0, 60)}`);
    }
    ...
    if (t && !t.startsWith("#") && !t.startsWith("-") && !t.startsWith("|") && !t.startsWith("<") && !t.startsWith("*")) {
      const dots = (noCode.match(/。/g) || []).length;
      if (dots >= 3) {
        hits.push(`  [長文段落:${dots}文] ${i + 1}: ${t.slice(0, 60)}`);
      }
    }
  });
  return hits;
