...
この問題を解決するのが **コンテナクエリ** です。親要素（コンテナ）の幅を基準にスタイルを切り替えられるので、カードはどこに置かれても「自分の周り」に合わせて見た目を変えられます。2023 年に全ブラウザが対応し、今は安心して本番で使えます。
...
下のデモは、**まったく同じ HTML のカード** を 200px の枠と 600px の枠に入れたものです。コンテナクエリにより、置かれた場所の幅だけでレイアウトが切り替わります。
...
<div style="display:flex;gap:16px;flex-wrap:wrap;background:#f8fafc;color:#1e293b;padding:16px;border-radius:8px;border:1px solid #e2e8f0;">
  <div style="background:white;color:#1e293b;padding:12px;border-radius:8px;border:1px solid #cbd5e1;">
    ...
    <div style="width:200px;container-type:inline-size;">
      <article style="display:flex;flex-direction:column;gap:8px;background:white;color:#1e293b;padding:12px;border:1px solid #e2e8f0;border-radius:6px;">
        <div aria-hidden="true" style="width:100%;aspect-ratio:1/1;background:#6366f1;border-radius:6px;"></div>
        <div>
          <h4 style="margin:0;font-size:14px;color:#1e293b;">山田 太郎</h4>
          <p style="margin:4px 0 0;font-size:12px;color:#475569;">フロントエンドエンジニア</p>
        </div>
      </article>
    </div>
  </div>
  ...
  <div style="background:white;color:#1e293b;padding:12px;border-radius:8px;border:1px solid #cbd5e1;flex:1;min-width:280px;">
    <p style="margin:0 0 8px;font-size:12px;color:#475569;">枠の幅: 600px（メイン想定）</p>
    <div style="width:100%;max-width:600px;container-type:inline-size;">
      <article style="display:flex;flex-direction:row;align-items:center;gap:16px;background:white;color:#1e293b;padding:16px;border:1px solid #e2e8f0;border-radius:6px;">
        <div aria-hidden="true" style="width:80px;height:80px;flex-shrink:0;background:#6366f1;border-radius:6px;"></div>
        <div>
          <h4 style="margin:0;font-size:18px;color:#1e293b;">山田 太郎</h4>
          <p style="margin:4px 0 0;font-size:14px;color:#475569;">フロントエンドエンジニア</p>
        </div>
      </article>
    </div>
  </div>
...
狭い側は縦並びでアイコンが大きく写り、広い側は横並びで情報が一列に並びます。カード側のコードは 1 種類しかありません。
