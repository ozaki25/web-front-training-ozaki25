...
/* OK: マウスクリック時だけ消し、キーボード時はしっかり見せる */
button:focus { outline: none; }
...
下のボタンとカードで、疑似クラスと `:has()` の挙動を触って確認できる。マウスで hover、Tab キーでフォーカス、チェックボックスを切り替えてみてほしい。

<div style="border:1px solid #e2e8f0;border-radius:8px;padding:16px;background:#f8fafc;color:#1e293b;">
  <p style="margin-top:0;color:#475569;">マウスでホバー / クリックと、キーボードの Tab キーでフォーカスしたときで枠の出方が違うことを見てほしい。</p>
  <button style="padding:8px 16px;border:1px solid #2563eb;background:white;color:#1e293b;border-radius:6px;cursor:pointer;">普通のボタン</button>
  <button disabled style="padding:8px 16px;border:1px solid #cbd5e1;background:#e2e8f0;color:#64748b;border-radius:6px;cursor:not-allowed;margin-left:8px;">disabled なボタン</button>

  <form style="margin-top:16px;padding:12px;background:white;color:#1e293b;border:1px solid #e2e8f0;border-radius:6px;">
    <label style="display:block;margin-bottom:8px;">
      <input type="checkbox" /> 利用規約に同意する
    </label>
    <label style="display:block;">
      メール: <input type="email" required placeholder="you@example.com" style="padding:4px 8px;border:1px solid #cbd5e1;border-radius:4px;background:white;color:#1e293b;" />
    </label>
    <p style="margin:8px 0 0;color:#64748b;font-size:0.9em;">メール欄が空 / 不正だと <code>:invalid</code>、入っていれば <code>:valid</code> に切り替わる。</p>
  </form>
</div>
