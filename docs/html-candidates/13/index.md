# カスタムフォーム UI — 見た目を変えてもアクセシビリティを保つ
...
実務のプロジェクトでは「デザイン通りの見た目にする」という要件がほぼ確実にあるため、フォーム部品のカスタマイズは避けて通れない作業です。
...
「ブラウザ標準」はブラウザが OS に合わせて描画したデフォルトの見た目で、「カスタマイズ後」がデザインに合わせて変更したものです。どちらもクリックで切り替わりますが、見た目の印象がまったく違います。
