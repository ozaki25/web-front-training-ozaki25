...
2024 年以降は主要ブラウザすべてで `light-dark()` 関数が使えます。ライト時の色とダーク時の色を並べて書くだけで、勝手にテーマに応じた値が選ばれます。
...
`color-scheme: light dark` は「このページはライトもダークも両対応です」という宣言です。これがあると `light-dark()` が機能するだけでなく、**フォーム部品・スクロールバー・デフォルトの選択ハイライト色まで**ブラウザが自動でダーク化してくれます。忘れやすいのに効果が大きい 1 行です。
...
Tailwind CSS v4 の `dark:bg-gray-900` も、内部的にはこの `@media (prefers-color-scheme: dark)` に展開されています（v4 では `@custom-variant` でクラス方式にも切替可能）。つまり `dark:` はマジックではなく、CSS の標準機能の薄いラッパーです。
