...
CSS が紙の文書を前提に設計されたため、「高さの中央に寄せる」という発想は想定外だった。長らく開発者は **ハック（本来の用途を曲げたテクニック）** で乗り切ってきた。古いコードで今も見かける 3 つを知っておこう。
...
**弱点**: 2 行以上になると破綻する。高さ固定が前提。

### ハック 2: `display: table-cell` + `vertical-align: middle`

HTML の `<table>` のセルは昔から縦中央寄せに対応していた。そこで `display: table-cell` でセルのふりをさせて `vertical-align: middle` を効かせる手が流行した。
...
**弱点**: レイアウトが「テーブル」扱いになり、他のスタイルに干渉する。
...
**弱点**: 通常の文書の流れから外れ、兄弟要素と重なる。モーダルなど本来浮かせたい要素には今も有効だが、そうでなければ柱 3 を選ぶ。
...
中央寄せしたテキストが長くなってコンテナからはみ出すと、視覚的にも読み上げ的にも困る。`overflow-wrap: anywhere`（単語の途中でも改行を許可）を添えておけば、URL のような長い文字列でも折り返される。
...
- Tailwind なら `grid place-items-center` / `flex items-center justify-center` で同じことができる
