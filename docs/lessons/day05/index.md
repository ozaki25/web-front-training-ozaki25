...
- `<dialog>` の `showModal()` でモーダルが作れる。フォーカストラップや Escape 対応も自動
- `<details>` / `<summary>` で JS なしの折りたたみ。`name` で排他制御も可能
- `popover` 属性でポップオーバー。外側クリックで閉じ、常に最前面
- 新機能の対応状況は **Baseline** で確認する
- 要素名を知っていれば AI に的確に指示できる
