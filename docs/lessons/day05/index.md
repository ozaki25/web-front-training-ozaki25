# Day 5: CSS ボックスモデルとレイアウト

## 今日のゴール

- すべての HTML 要素が「ボックス（箱）」であることを理解する
- margin、padding、border の違いを説明できる
- `box-sizing: border-box` の意味と使い方を知る
- `display` プロパティによる表示の違いを理解する

## ボックスモデル — すべては「箱」

ブラウザは HTML のすべての要素を「四角い箱（ボックス）」として扱います。これを**ボックスモデル**と呼びます。Day 4 で CSS の基本を学びましたが、レイアウトを理解するにはこのボックスモデルの理解が不可欠です。
...
ブラウザの開発者ツール（DevTools）で確認してみましょう。要素を右クリック →「検証」で開けます。「Computed」タブや要素のボックスモデル図で、content・padding・border・margin の値が確認できます。
...
ブラウザで確認すると、ブロック要素は縦に積み重なり、インライン要素はテキストの流れの中に横に並ぶことがわかります。
...
**次のレッスン**: [Day 6: Flexbox](/lessons/day06/)
