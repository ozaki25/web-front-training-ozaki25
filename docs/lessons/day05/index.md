...
- 確認ダイアログ、折りたたみ、ポップオーバーが HTML だけで作れることを知る
- これらの HTML 要素がアクセシビリティやキーボード操作を自動で処理してくれることを知る
- 新しい HTML 機能を使うときの判断基準（Baseline）を知る

## よく見る UI パーツ、実は HTML だけで作れる

普段使っている Web サイトには、こんな UI パーツがよく出てきます:

- 「本当に削除しますか？」のような**確認ダイアログ**
- FAQ ページの**折りたたみ**（クリックで開閉する）
- ヘルプやメニューの**ポップオーバー**（浮き上がって表示される）

こうした UI は、以前は JavaScript で頑張って作る必要がありました。しかし今は HTML の要素や属性だけで実現できます。しかも、キーボード操作やスクリーンリーダー対応が自動で付いてきます。

AI にこうした UI を作らせると、JavaScript ベースの古い作り方で実装してくることがあります。HTML だけでできると知っていれば、「`<dialog>` を使って」のように的確な指示が出せます。

## 新しい HTML 機能と Baseline

HTML には今も新しい要素や属性が追加されています。ただし、すべてのブラウザですぐに使えるとは限りません。

その判断基準が **Baseline** です。Chrome・Safari・Firefox・Edge の対応状況をもとに、3 段階で分類されています。

| ステータス | 意味 |
|-----------|------|
| **Limited availability** | 一部のブラウザでしか使えない。本番投入には慎重に |
| **Newly available** | 主要ブラウザすべてで使えるようになって 30 ヶ月未満。基本的に本番で使える |
| **Widely available** | 主要ブラウザすべてで使えるようになって 30 ヶ月以上。安心して使える |
...
確認ダイアログやログインフォームのポップアップなど、背後の操作をブロックして表示する UI を**モーダルダイアログ**と呼びます。

モーダルを JavaScript で自作しようとすると、考慮すべきことが多くなります:
...
複数の `<details>` に同じ `name` 属性を付けると、1つ開くと他が自動的に閉じる排他的なアコーディオンになります。
...
ヘルプやメニューなど、ボタンを押すと浮き上がって表示される UI です。
...
| 用途 | 確認ダイアログ、フォーム | ツールチップ、メニュー、通知 |

「ユーザーに回答を求める」なら dialog、「補助的な情報を表示する」なら popover、と覚えておけば迷いません。

## まとめ

- `<dialog>`: `showModal()` で確認ダイアログが作れる。フォーカストラップ、Escape 対応、背景暗転が自動
- `<details>` / `<summary>`: JavaScript なしでアコーディオンが作れる。`name` 属性で排他制御も可能
- `popover` 属性: ポップオーバーが作れる。外側クリックで閉じる、常に最前面に表示
- 新しい HTML 機能を使うときは **Baseline** で対応状況を確認する
- AI はこれらを JavaScript ベースで実装することがある。HTML 要素の名前を知っていれば、的確に指示できる
