...
`"use cache"` が `fetch` 専用ではない点も大きな違いです。DB へのクエリや ORM など、`fetch` を使わない取得でも、関数に付ければ同じように保存できます。従来は `fetch` だけがキャッシュを内蔵し、それ以外は `unstable_cache` が必要でしたが、取得手段によらず `"use cache"` 1 つに揃いました。
...
従来モデルでは、ページが静的なら Next.js が自動で HTML を保存しました。これが Full Route Cache です。ただし**ページ単位**で、1 か所でも動的な部分があるとページ全体が動的になり、保存されませんでした。在庫のように最新が要る部分が 1 つあるだけで、変わらない商品説明やヘッダーまで毎回作り直され、ページ全体が遅くなります。これが従来の困りごとでした。

新モデルの一番大きな変化がここです。1 枚のページの中で、静的な部分と動的な部分を**混ぜられる**ようになりました。変わらない部分は保存して速いまま、変わる部分だけアクセスごとに最新にできます。1 つの動的な部分のためにページ全体を諦める、ということがなくなります。

自動の静的化はなくなり、どちらにするかは部分ごとに開発者が選びます。

- 保存していい部分（商品説明など） → `"use cache"` を付ける（静的。すぐ表示される）
- アクセスごとに最新が要る部分（在庫数など） → `<Suspense>` で囲む（動的。あとから届く）

商品ページで見てみます。商品説明は保存し、在庫数だけアクセスごとに取ります。
...
`ProductDescription` に `"use cache"` を付けると、その部分は保存されてすぐ表示されます。`StockCount` は囲むだけでキャッシュせず、アクセスごとに取得して、届くまでは `<Suspense>` の仮表示が出ます。商品説明は速いまま、在庫数だけを最新にできる。これが「ページ単位から部品単位へ」の中身です。
