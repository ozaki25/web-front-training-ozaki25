...
- **`"use cache"` の中で `cookies()` などは読めない**: キャッシュは全員で共有される保存なので、ログイン情報のようにユーザーごとに変わるものは使えません。必要なときは、キャッシュの外側で読んで引数として渡します。
- **キャッシュは自動では新しくならない**: `cacheTag` を付けておき、データを変えたら `revalidateTag` で消します。この消し込みを忘れると、いつまでも古い内容が出ます。
