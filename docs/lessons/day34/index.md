...
考え方も切り替わります。従来はページ単位で「静的か動的か」を考えていました。新モデルでは、1 枚のページを「先に出す土台」と「後から届く穴」に分け、その境界を `<Suspense>` で自分で引く、と考えます。`<Suspense>` は、土台に動的な穴を開けるための目印です。

そのうえで新しく気にするのが、境界を**どこに引くか**です。遅い部分だけを囲めば、残りは土台として先に出せます。ページ全体を 1 つの `<Suspense>` で囲むと、全部が後回しになり、土台を先に出す意味がなくなります。
