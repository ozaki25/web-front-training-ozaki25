...
従来モデルは条件がそろえば自動で保存しましたが、新モデルは `"use cache"` を**書いたものだけ**を保存します。書かなければ保存されません。

`"use cache"` が `fetch` 専用ではない点も大きな違いです。DB へのクエリや ORM など、`fetch` を使わない取得でも、関数に付ければ同じように保存できます。

従来は `fetch` だけがキャッシュを内蔵し、それ以外は `unstable_cache` が必要でしたが、取得の仕方が違っても `"use cache"` 1 つで済むようになりました。

`cacheLife` には、数字ではなく**プロファイル名**を渡します。名前ごとに「どれくらいの間隔で作り直すか」があらかじめ決まっているので、業務の感覚に近い名前を選ぶだけです。
...
従来モデルでは、ページが静的なら Next.js が自動で HTML を保存しました。これが Full Route Cache です。

ただし選べるのは**ページ単位**で、丸ごと保存して使い回す（速いが古くなりうる）か、丸ごと毎回作り直す（最新だが遅い）かの二択でした。在庫のように最新が要る部分が 1 か所あるだけで、ページ全体を毎回作り直すことになり、変わらない商品説明やヘッダーまで遅くなります。

新モデルは、この二択をやめます。自動の静的化はなくなり、代わりに 1 枚のページの中で、保存する部分と毎回作り直す部分を**混ぜられる**ようになりました。

保存する部分に `"use cache"` を付け、毎回作り直す部分は `<Suspense>` で囲みます。
...
考え方も切り替わります。従来はページ単位で「静的か動的か」を考えていました。

新モデルでは、1 枚のページを「先に出す土台」と「後から届く穴」に分け、その境界を `<Suspense>` で自分で引く、と考えます。`<Suspense>` は、土台に動的な穴を開けるための目印です。

そのうえで新しく気にするのが、境界を**どこに引くか**です。

遅い部分だけを囲めば、残りは土台として先に出せます。ページ全体を 1 つの `<Suspense>` で囲むと、全部が後回しになり、土台を先に出す意味がなくなります。
