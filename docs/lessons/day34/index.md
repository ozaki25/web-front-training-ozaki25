# Day 34: キャッシュの新モデル — 4 つのキャッシュがどう変わるか

## 今日のゴール

- 新モデルでも、4 つのキャッシュのうち 2 つはそのままだと分かる
...
つまり、変わるのは**サーバー側の 2 つ（Data Cache と Full Route Cache）の使い方**で、ブラウザ側と重複排除はそのままです。変わる 2 つも、やることは `"use cache"` 1 つにまとまります。
...
`"use cache"` が `fetch` 専用ではない点も大きな違いです。DB へのクエリや ORM など、`fetch` を使わない取得でも、関数に付ければ同じように保存できます。従来は `fetch` だけがキャッシュを内蔵し、それ以外は `unstable_cache` が必要でしたが、取得の仕方が違っても `"use cache"` 1 つで済むようになりました。
...
「作り直す間隔」はサーバーが裏で作り直す頻度、「期限」はそれを過ぎたら必ず作り直す上限です。このほかに「クライアントが再確認なしに使える時間」もあり、`seconds` だけ 30 秒、それ以外は 5 分です。

プリセットに合わないときは、名前の代わりに秒数のオブジェクトを渡して細かく決められます。
...
## Full Route Cache — 「ページ丸ごと」から「部品ごと」へ

従来モデルでは、ページが静的なら Next.js が自動で HTML を保存しました。これが Full Route Cache です。ただし選べるのは**ページ単位**で、丸ごと保存して使い回す（速いが古くなりうる）か、丸ごと毎回作り直す（最新だが遅い）かの二択でした。在庫のように最新が要る部分が 1 か所あるだけで、ページ全体を毎回作り直すことになり、変わらない商品説明やヘッダーまで遅くなります。

新モデルは、この二択をやめます。自動の静的化はなくなり、代わりに 1 枚のページの中で、保存する部分と毎回作り直す部分を**混ぜられる**ようになりました。保存する部分に `"use cache"` を付け、毎回作り直す部分は `<Suspense>` で囲みます。
...
商品説明は保存したまま、在庫数だけを最新にできます。これが「ページ丸ごとから部品ごとへ」の中身です。

Full Route Cache まわりの書き方も、こう対応します。

| 従来モデル | 新モデル |
|----------|---------|
| Next が自動で静的化 | `"use cache"` を付けた部分だけ静的 |
| 動的にするには `connection()`（ページ全体が動的） | キャッシュせず `<Suspense>` で囲む（その部分だけ動的） |
| ページ単位（静的か動的の二択） | 部品単位（1 ページに同居） |
...
`<Suspense>` は、静的な土台に動的な穴を開けるための目印だと考えると分かりやすいです。保存しない非同期データは、`<Suspense>` で囲むか `"use cache"` を付けるかの**どちらかが必須**で、従来のように Next.js が自動でどちらかに決めることはありません。
...
判断が自動から自分の手に移ったぶん、これまで意識しなくてよかったことに気をつける必要が出てきます。

- **非同期データごとに、キャッシュするかを必ず決める**: 保存するなら `"use cache"`、しないなら `<Suspense>`。どちらも付け忘れるとビルドが通りません。従来のように「勝手にどちらか」にはなりません。
- **`"use cache"` の中で `cookies()` などは読めない**: キャッシュは全員で共有されるので、ログイン情報のようにユーザーごとに変わるものは使えません。必要なときは、キャッシュの外側で読んで引数として渡します。
- **キャッシュは自動では新しくならない**: `cacheTag` を付けておき、データを変えたら `revalidateTag` で消します。これを忘れると、いつまでも古い内容が出ます。
