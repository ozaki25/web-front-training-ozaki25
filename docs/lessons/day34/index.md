...
考え方もここで切り替わります。従来はページ単位で「静的か動的か」を考えていましたが、新モデルでは「1 枚のページを、先に出す土台と後から届く穴に分け、その境界を自分で引く」と考えます。ここで新しく気にするのは、境界を**どこに引くか**です。遅い部分・最新が要る部分だけを `<Suspense>` で囲みます。ページ全体を 1 つの `<Suspense>` で囲むと、全部が後回しになり、土台を先に出す意味がなくなります。
