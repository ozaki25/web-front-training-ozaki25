...
sequenceDiagram
    ...
    B->>S: ① URL を送る
    S-->>B: ② HTML を返す
    Note over B: HTML を読み始める
    B->>S: ③ CSS を要求（link タグを発見）
    S-->>B: ④ CSS を返す
    B->>S: ⑤ JS を要求（script タグを発見）
    S-->>B: ⑥ JS を返す
    Note over B: 画面を組み立てて表示
```

1. ブラウザのアドレスバーに URL を入力すると、ブラウザは **サーバーにリクエスト** を送ります
2. サーバーはまず **HTML** を返します
3. ブラウザは HTML を読み始め、その中で `<link>` タグや `<script>` タグを見つけると、**CSS や JavaScript を追加でサーバーに要求** します
4. すべてのファイルが揃ったら、ブラウザが **画面を組み立てて表示** します
...
今はコードの中身を読み解く必要はありません。ここで知っておきたいのは、HTML の中で CSS と JavaScript のファイルを読み込んでいる（`<link>` と `<script>`）ということです。この3つが組み合わさって、ブラウザの画面が出来上がります。
