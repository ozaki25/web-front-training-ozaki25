...
Day 21 でコンポーネントの基本を学びました。しかし、あのままでは毎回同じ内容しか表示できません。
...
Day 17〜20 で学んだ TypeScript の `interface` や `type` を使って、props の型を定義します。
...
Day 18 で学んだ Optional（`?`）やリテラル型のユニオンがここで活きています。

## 分割代入で props を受け取る

Day 11 で学んだ分割代入（destructuring）を使うと、`props.` を毎回書かなくて済みます。
...
**次のレッスン**: [Day 23: state とイベント処理](/lessons/day23/)
