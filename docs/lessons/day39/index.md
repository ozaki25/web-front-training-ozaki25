...
こうした分野を支えてきたのは、計算そのものに特化した **C++** という言語です。近年は同じ系統の言語として **Rust** も広く使われるようになりました。

パソコンにインストールするソフトの多くも、この系統の言語で書かれています。
...
WebAssembly は、特定の会社の製品でも、流行り廃りのあるライブラリでもありません。HTML や CSS の仕様を管理している **W3C** という標準化団体が仕様を定めた、正式な Web 標準です。

2019 年に標準となったとき、W3C は「HTML・CSS・JavaScript に続く、Web の 4 つ目の言語」と紹介しました。
...
変換元にできる言語は、C++ や Rust に限りません。Go、C#、Python など幅広い言語が対応しています。

::: details 変換元になる言語の例（参考）
...
ブラウザに届いたあとも CPU 向けの機械語へ変換する工程は残りますが、機械向けの形から始めるぶん、文字のコードを解釈するより速く終わります。

届くファイルの実体は、0 と 1 が並んだ**バイナリ形式**で、拡張子は `.wasm` です。

- JavaScript と違い、開発者ツールで中身をそのまま読むことはできない
- C++ や Rust を普通にコンパイルすれば OS のファイルやネットワークに直接アクセスできるが、WebAssembly への変換でその権限は失われ、JavaScript と同じ**サンドボックス**（ブラウザが用意した、隔離された実行環境）の中で動く
- **画面（DOM）や fetch などの Web API にも直接は触れない**: やり取りできるのは整数や浮動小数点数といった数値だけで、画面の書き換えやデータの取得は JavaScript を経由する必要がある
...
画面に直接触れない WebAssembly は、単独ではアプリになりません。JavaScript を置き換えるのではなく、**役割を分けて共存する**関係です。
...
::: details 「キャンバス」とは何か（参考）
本文で「キャンバス」と呼んでいる領域が **Canvas**（`<canvas>` 要素）です。同じ四角形を表示するのでも、`<div>` を並べて作る普通の画面と Canvas では、仕組みがまったく違います。

普通の画面の四角形は `<div>` のような要素で、ブラウザがずっと「ここに四角形がある」と覚えています。あとから位置や色を変えることもできますし、クリックされたらブラウザがどの四角形かを教えてくれます。

Canvas はそうではありません。中身はただの色のついた点（ピクセル）の集まりで、四角形や丸を描いても、描いた瞬間に「絵」になるだけです。

「ここに四角形があった」という情報はブラウザに残らないので、あとから 1 つだけ動かすことはできません。何かを動かしたいときは、画面をまるごと消してから、新しい位置でもう一度全部を描き直します。

```js
const canvas = document.querySelector("canvas");
const ctx = canvas.getContext("2d"); // Canvas に描き込むための窓口

function draw() {
  ctx.clearRect(0, 0, canvas.width, canvas.height); // まず全部消す
  ctx.fillRect(x, 10, 100, 50); // x は図形の現在位置。動かすたびに変わる
}
```

クリックされたのがどの図形かも、ブラウザは教えてくれません。クリックされた座標が、自分が描いた図形の内側にあるかどうかを自分で計算する必要があります。

Figma のようなアプリの内部実装が複雑になる理由のひとつは、こうした判定をすべて自前で持っていることです。

そして Canvas への命令（`fillRect` など）は、DOM の書き換えや fetch と同じく、ブラウザが JavaScript に開放している機能（**Web API**）の一種です。WebAssembly は Web API を直接呼べないので、実際の流れは「WebAssembly が次に描く内容を計算し、JavaScript が Canvas に命令してピクセルを塗る」という形になります。

Canvas も、JavaScript と WebAssembly が役割を分けて共存する具体例のひとつです。
:::
...
逆に、フォームや一覧が中心の普通の Web アプリでは、出番はほぼありません。画面の組み立てはこれまでどおり JavaScript のほうが素直に書けますし、変換元のコードが大きいと `.wasm` が数 MB 規模になって、読み込みがかえって遅くなることもあります。
