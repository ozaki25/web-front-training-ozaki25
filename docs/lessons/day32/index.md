...
**ページ本体はデフォルトで毎回新しくなります。** 一覧から詳細へ進むような通常の遷移では、ページの中身は古い保持データではなく最新が表示されます。

一方、**全ページで共通のレイアウトは保持される**ので、取り直しが省かれて遷移が速くなります。

別のページへ遷移したとき、何が再利用され、何が取り直されるかを図にすると次のようになります。

<svg viewBox="0 0 560 250" role="img" aria-label="商品一覧から商品詳細へ遷移する図。共有レイアウトのヘッダー（チェック印・実線）は両方の画面で保持されて再利用され、ページ本体（矢印印・破線）だけが商品一覧から商品詳細へ取り直される" style="width:100%;height:auto;max-width:560px;display:block;margin:16px auto;">
  <defs>
  ...
  <rect x="40" y="36" width="190" height="54" fill="none" stroke="#16a34a" stroke-width="2"/>
  <rect x="40" y="93" width="190" height="93" fill="none" stroke="#dc2626" stroke-width="2" stroke-dasharray="6 4"/>
  ...
  <rect x="330" y="36" width="190" height="54" fill="none" stroke="#16a34a" stroke-width="2"/>
  <rect x="330" y="93" width="190" height="93" fill="none" stroke="#dc2626" stroke-width="2" stroke-dasharray="6 4"/>
  ...
  <rect x="60" y="218" width="16" height="16" rx="3" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/>
  <text x="68" y="230" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="700" fill="#166534">✓</text>
  <text x="82" y="230" font-family="sans-serif" font-size="12" fill="#1e293b">実線は保持（遷移しても再利用）</text>
  <rect x="330" y="218" width="16" height="16" rx="3" fill="#fee2e2" stroke="#dc2626" stroke-width="2" stroke-dasharray="4 3"/>
  <text x="338" y="230" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="700" fill="#b91c1c">↻</text>
  <text x="352" y="230" font-family="sans-serif" font-size="12" fill="#1e293b">破線は毎回取り直す</text>
</svg>

実線で囲んだレイアウトは遷移してもそのまま使い回され、破線で囲んだページ本体は捨てて取り直されます。だから遷移は速い一方、レイアウトに置いたデータは古いまま残ることがあります。
