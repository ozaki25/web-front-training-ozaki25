...
どれだけ横幅に余裕があっても、ブロック要素は上から下に並びます。Web ページで要素を「横に並べたい」場面は多いのに、CSS にはそのための仕組みが長い間ありませんでした。
...
float のような親の高さ問題は起きません。しかし inline-block には別の厄介な問題がありました。

要素と要素の間に <strong>意図しない隙間</strong> ができるのです。
...
横に並べるだけのことにこれだけの工夫が必要だったのは、float も inline-block も横並びのための仕組みではなかったからです。
...
CSS で上下中央に配置するのは、昔は `position: absolute` と `transform: translate(-50%, -50%)` を組み合わせる必要がありました。「CSS で中央寄せが難しい」はインターネット上で長年ネタにされてきた話です。

Flexbox がこれを過去のものにしました。
...
最後の行のカード 2 枚が、残りのスペースを分け合って幅が広がってしまいます。これはバグではなく、flex の仕様です。

flex は 1 行ずつ独立して動く仕組みなので、行をまたいで列幅を揃えることができません。
...
grid なら親の 1 行で列の構造が決まり、子には何も書かなくて済みます。ナビは `1fr` の列に入るので自動的に右端に寄ります。

レイアウトの意図がコードに現れるので、後から読んだときにも構造がわかりやすいです。
...
タグの幅は中身のテキストによって変わります。「HTML」は短く、「アクセシビリティ」は長い。

これを grid で固定幅のトラックに入れると、短いタグの横に無駄なスペースが生まれます。子の中身がサイズを決める場面は、交渉型の flex がぴったりです。
...
そして flex と grid も役割が違います。flex は子がサイズを交渉する仕組みなので柔軟ですが、子が増えたり中身が変わると予想外の結果になりがちです。

grid は親が構造を先に決めるので予測しやすく、意図がコードに残ります。「とりあえず flex」ではなく、まず grid で書けないか考えてみる——それが今の定番の考え方です。

## まとめ

- CSS の要素はデフォルトで縦積みで、横並びには工夫が必要だった
- **float** / **inline-block** は制約が多く、現在は **flex** と **grid** を使う
- **grid** は親がレイアウトを定義、**flex** は子がサイズを交渉するので、まず grid を検討する
