# Day 2: セマンティック HTML — ページの構造を伝えるタグ
...
HTML には、ページの構造を伝えるための仕組みがいくつも用意されています。
...
深い階層から浅い階層に**戻る**のは問題ありません。上の例でも、h3（肉じゃが）の後に h2（洋食）に戻っています。

本の目次でも節が終われば次の章に移るのは自然です。
...
- HTML タグは見た目でなくページの構造を機械に伝えるもの
- **ランドマーク**（header, nav, main 等）でページの「地図」を作る
- **見出し階層**（h1〜h6）でページの「目次」を作り、レベルは飛ばさない
- **セクショニング**: section はまとまり、article は独立したコンテンツ
- React のコンポーネント分割では見出しレベルが壊れやすい
- div はレイアウト用で、意味のあるタグが当てはまらないときに使う
