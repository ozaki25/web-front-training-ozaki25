...
注意点は隠し方です。`display: none` だとスクリーンリーダーからも消えます。`sr-only` という手法なら「目には見えないが支援技術からはアクセスできる」状態にできます（Tailwind CSS にも同名のクラスがあります）。ラジオボタンも同じ考え方です。
...
この問題を標準の仕組みで解決する **カスタマイザブル `<select>`** が策定中です。`appearance: base-select` を指定するだけで、ドロップダウンも選択肢もすべて CSS でスタイリングできます。
...
ネイティブの `<select>` のままなので、キーボード操作もフォーム送信もそのまま。Chrome 系ブラウザで見ている方はデモで確認できます（Safari / Firefox では通常の `<select>` になります）:
...
仕組みはチェックボックスと同じで、`<input type="radio">` を隠して `<label>` のカード全体をクリック可能にしています。
...
`<input type="file">` も同じ考え方で、ボタンの見た目だけ変えられます。ファイル選択ダイアログ自体はネイティブのままです。
