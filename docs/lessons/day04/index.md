...
チェックボックスやラジオボタンなどのフォーム部品は、機能としては十分です。キーボード操作もスクリーンリーダー（画面を音声で読み上げるソフト）も対応しています。

ただし**見た目はブラウザと OS が決める**ので、デザインとは合いません。実務では「デザイン通りの見た目にする」要件がほぼ確実にあり、カスタマイズは避けられません。

問題は、**見た目を変えるとアクセシビリティが壊れやすい**こと。今日はこの問題と解決策を見ていきます。
...
本物の `<input>` を目に見えないように隠して、隣の要素で見た目を作ります。クリックやキー操作は裏の `<input>` が受け取るので、キーボード操作もフォーム送信もそのまま動きます。

注意点が 2 つあります:

- **隠し方**: `display: none` だとスクリーンリーダーからも消えます。`sr-only` という手法なら「目には見えないが支援技術からはアクセスできる」状態にできます（Tailwind CSS にも同名のクラスがあります）
- **フォーカスリング**: キーボードで Tab 移動したとき、どこにフォーカスがあるか見える状態を維持する必要があります。上のデモで Tab キーを押すと青い枠線が表示されるのがそれです

ラジオボタンやカード型 UI、ファイル選択ボタンなども同じ考え方でカスタマイズできます。

## div で作り直すのはアンチパターン

`<div>` と JavaScript でゼロから作ればいいのでは？これは**アンチパターン**です。`<div>` で見た目だけ作ると、以下がすべて壊れます:
...
自前で全部再現するのは非常に難しく、ブラウザが無料で提供してくれる機能を作り直しているだけです。**ネイティブの要素を使い、見た目だけ CSS で変える。** これが鉄則です。

## select とコンボボックスは別格

チェックボックスやラジオボタンは「隠して被せる」でうまくいきます。ところが **`<select>`（ドロップダウン）は話が別**です。

ドロップダウン部分は OS のネイティブ UI なので、CSS ではほぼカスタマイズできません。色を変えたりアイコンを付けたりするには、結局 `<div>` でゼロから組むことになります。

**コンボボックス**（テキスト入力 + ドロップダウン）はさらに厄介です。キーボード操作、ARIA 属性の管理、フォーカス制御など、求められる振る舞いが膨大で、経験豊富なエンジニアでも自力実装は大変です。

select やコンボボックスは「素朴なカスタマイズ」が通用しない部品だと覚えておきましょう。
...
この問題を標準の仕組みで解決する **カスタマイザブル `<select>`** が策定中です。CSS で `appearance: base-select` を指定するだけで、ドロップダウンも選択肢もすべてスタイリングできるようになります。ネイティブの `<select>` のままなので、キーボード操作もフォーム送信もそのまま動きます。

Chrome 系ブラウザで見ている方はデモで確認できます（Safari / Firefox では通常の `<select>` になります）:
...
今日のプロジェクトでカスタマイザブル `<select>` はまだ使えません。現実解は 2 つです:

- **ヘッドレス UI ライブラリを使う**（後述）
- **見た目で妥協して標準の `<select>` を使う**

「いずれネイティブで解決される方向」と頭に入れつつ、今はライブラリに頼る、という判断ができればOKです。

## 応用例

同じ考え方で、こんな UI もカスタマイズできます。

### カード型のラジオボタン

カード全体がラジオボタンになっている UI です。裏にある `<input type="radio">` を隠して、カード全体をクリック可能にしています。
...
チェックボックスやラジオボタンは CSS で対応できますが、select のように「ゼロから組む必要がある」部品もあります。そこで使うのが**ヘッドレス UI ライブラリ**です。

「ヘッドレス（headless）」= 見た目がない。つまり:

- **ライブラリが提供するもの**: キーボード操作、ARIA 属性の管理、フォーカス制御
- **開発者が作るもの**: 見た目（CSS）

React Aria（Adobe 製）などのライブラリを使えば、複雑な部品でもスタイリングだけに集中できます。**振る舞いとアクセシビリティはライブラリに任せ、見た目だけ自分で作る。** この分担を覚えておきましょう。
