...
フォーム部品（チェックボックス、ラジオボタンなど）は、機能としては十分です。キーボード操作もスクリーンリーダーも対応しています。

ただし**見た目はブラウザと OS が決めます**。デザインとは合わないことがほとんどです。

実務では「デザイン通りの見た目にする」要件がほぼ確実にあります。フォーム部品のカスタマイズは避けられません。
...
裏に本物がいるので、キーボード操作もフォーム送信もそのまま動きます。
...
経験豊富なエンジニアでも自力実装は大変です。「素朴なカスタマイズ」が通用しない部品だと覚えておきましょう。
...
この問題を標準の仕組みで解決する**カスタマイザブル `<select>`** が策定中です。
