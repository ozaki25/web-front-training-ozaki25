...
フォーム部品の見た目はブラウザと OS が決めています。そのためデザインと合わないことがほとんどで、実務ではカスタマイズが必要になります。
...
どちらもクリックで切り替わりますが、見た目の印象がまったく違います。AI にフォームを作らせると、カスタマイズされた見た目のコードが出てくることも多いはずです。では、そのコードの品質はどう判断すればいいのでしょうか。

## 見た目は動いていても、ちゃんと動いているとは限らない

HTML 標準のフォーム部品（`<input type="checkbox">` など）には、見た目以上の機能が組み込まれています:

- クリックで切り替わる
- キーボード（Space キー）でも切り替えられる
- Tab キーでフォーカスを移動できる
- スクリーンリーダー（画面読み上げソフト）が「チェックボックス」と読み上げる
- `<form>` で送信すると値が送られる
- ブラウザの自動入力に対応している

見た目だけを気にしてフォーム部品を作り直すと、これらが失われます。
...
クリックで動いているから大丈夫、ではありません。キーボードで操作する人、スクリーンリーダーを使う人にとっては壊れた状態です。AI が生成したコードでも、この品質の差は起こりえます。

大事なのは、**見た目を変えても、HTML 標準の機能を壊さないこと**です。

## やり方は部品によって違う

フォーム部品のカスタマイズは、すべて同じ方法ではうまくいきません。部品によって難易度が大きく違います。
...
`<select>` のドロップダウン部分は OS のネイティブ UI です。CSS ではほぼカスタマイズできません。
...
`<select>` のカスタマイズ問題を標準の仕組みで解決する**カスタマイザブル `<select>`** が策定中です。
...
将来的にはこれが標準の解決策になりますが、まだすべてのブラウザで使えるわけではありません。
