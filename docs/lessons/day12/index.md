...
| 方法 | トリガー | 向いている動き | 制御 |
|------|---------|-------------|------|
| CSS `transition` | ホバー、クラスの変更 | A → B の一回の変化 | CSS だけで完結 |
| CSS `animation` | 自動、ページ表示時 | ループ、複数段階 | CSS だけで完結 |
| CSS Scroll-driven | スクロール | スクロール量に連動 | CSS だけで完結 |
| JS（Web Animations API / ライブラリ） | ユーザー操作、条件分岐 | 途中停止、速度変更、状態に応じた分岐 | プログラムで細かく制御 |
| Canvas / WebGL | — | ゲーム、粒子エフェクト | 全てを自分で描画 |
...
ボタンにマウスを乗せると色が変わる。メニューがスッと開く。

こうした「状態 A から状態 B への変化」を滑らかに見せるのが `transition` です。
...
イージングは体感に効きます。`linear`（等速）だと機械的に、`ease-out`（最後がゆっくり）だと自然に見えます。

迷ったら `ease-out` で十分です。

## animation — トリガーがなくても動く

読み込み中にくるくる回るスピナー。ページを開いた瞬間にふわっと現れるカード。

こうした「状態変化をトリガーにしない動き」や「ループする動き」には `@keyframes` と `animation` を使います。

```css
/* fade-in: from → to の 2 段階 */
@keyframes fade-in {
...
`@keyframes` は「どの時点でどの状態にいるか」を書いた台本です。`from`/`to` は `0%`/`100%` と同じ意味で、2 段階ならこれで十分です。

途中に緩急をつけたいときはパーセントで中間地点を指定します。上のスピナーは前半 40% で 300deg 回り、残り 60% で 60deg だけ回るので、速い → 遅いの緩急がつきます。

<div class="c056-demo">
  <div style="display:flex;gap:24px;align-items:center;flex-wrap:wrap">
    <div style="display:flex;flex-direction:column;align-items:center;gap:8px">
      <div class="c056-fadein-card" id="c056-fadein-card">fade-in カード</div>
      <span class="c056-demo-note">from/to（一回だけ）</span>
    ...
    <div style="display:flex;flex-direction:column;align-items:center;gap:8px">
      ...
      <span class="c056-demo-note">パーセント指定（ループ）</span>
    </div>
  </div>
</div>
...
迷ったら「トリガーがあるか」で判断します。

`:hover` やクラスの付け外しなど、何かのトリガーで A → B に変わるなら `transition`。それ以外は `animation` です。
...
- 「要素を動かす」なら `top`/`left` ではなく `transform: translate()`
- 「見え隠れ」なら `display` の切り替えではなく `opacity`

この 2 つに寄せるだけで、体感の滑らかさが変わります。
...
- 多くの UI アニメーションは CSS だけで十分
- **transition** は状態変化をトリガーに A → B を滑らかにつなぐ
- **animation** はトリガー不要でループや複数段階の動きに使う
- `transform` と `opacity` に寄せると軽く、`width` や `top` は重い
