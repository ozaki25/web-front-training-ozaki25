...
- DOM（Document Object Model）が何かを理解する
- JavaScript で HTML 要素を取得・作成・変更・削除できる
- イベントリスナーでユーザー操作に反応できる
- DOM 操作の煩雑さを体感し、React のような宣言的 UI の価値を理解する前段とする
...
> **セキュリティの注意**: `innerHTML` はユーザーが入力した文字列をそのまま入れると、悪意あるスクリプトが実行される危険（XSS 攻撃）があります。ユーザー入力を表示する場合は `textContent` を使いましょう。
...
コードを見てください。アクセシビリティのポイントがいくつかあります。
...
先ほどの ToDo リストのコードを振り返ってみてください。やっていることは「テキストをリストに追加する」「削除ボタンで消す」というシンプルな機能です。しかし:
...
今日の DOM 操作の煩雑さを体感しておくことで、後で React を学ぶときに「なぜ React が必要なのか」が実感として理解できるはずです。
