...
2020年、note で記事詳細ページのソースから投稿者の IP アドレスが確認できる状態になっていました。画面には出ていません。対処は API のレスポンスから IP アドレスのデータを削除することで、この節でいう「渡す前に絞る」そのものです。

この状態は1年4か月続いていて、気づいたのは社内ではなく利用者でした。期待どおりに画面が動いていたからです。

> 参考: [noteでソースコードからIPアドレスが確認できた事態に関する追加報告とお詫び](https://note.jp/n/n2115642a4e45)（note株式会社）

ある政府系のサイトでも、ページに埋め込まれたデータの中に、画面には出していない情報が入ったままになっていました。外部の開発者がそれを見つけて指摘しています。
