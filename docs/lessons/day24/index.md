...
- JSX の中で条件に応じて表示を切り替えられるようになる
- `map` で配列をリスト表示できるようになる
- `key` の役割と仕組みを理解する
...
> **注意**: `&&` の左側に数値を直接書くと意図しない表示になることがあります。`{0 && <span>...</span>}` は `0` が画面に表示されてしまいます。数値の場合は `{count > 0 && ...}` のように必ず比較式にしましょう。
...
> **ポイント**: データに `id` がある場合は `id` を使いましょう。`id` がなく、リストが固定で並び替えや削除がない場合に限り、インデックスを key にしても問題ありません。
