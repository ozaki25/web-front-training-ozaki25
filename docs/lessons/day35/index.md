...
- 厚くする対象を、機能ごとと層ごとの 2 つの見方で考えると知る
- AI が量産できる今も、配分の考え方は変わらないと知る

## 全部はテストできない

「全部テストすれば安心」と思いたくなります。でも、あらゆる入力・あらゆる組み合わせを試すのは、そもそも不可能です。

入力欄が 1 つあるだけでも、文字数・文字種・記号・空・超長文…と、あり得る入力は無限にあります。組み合わせれば天文学的な数になり、現実的な時間では終わりません。

「**全数テストは不可能**」は、テストの原則のひとつです。だからプロも「全部」ではなく「どこに力を入れるか」を決めます。

この**力のかけどころを決めるのがテスト計画**です。見方は 2 つあります。

- **機能で見る**: どの機能を厚く／薄くするか
- **層で見る**: どの種類のテストをどの割合で持つか

## どの機能を厚くするか

力の入れどころは、リスクで決めます。この進め方を**リスクベースドテスト**と呼びます。

リスクの大きさ、つまりリスクレベルは、2 つの掛け合わせで決まります。

- **リスクの影響**: そこが壊れたときの痛手。決済が止まるのと、端の表示が少しずれるのとでは重みが違う
...
この 2 軸の図を**リスクマトリクス**と呼びます。大小で見積もる定性的なやり方です。

数値で出すなら、リスクレベルは可能性と影響の掛け算になります。読み方はシンプルです。

- **右上の最重点**: 決済・ログイン・複雑な割引ロジック。境界値や組み合わせまで厚く
...
- **コアドメイン**: 複雑で、競合との差になる中心。決済・料金計算・独自の推薦など。壊れると事業が止まり、他社から買うこともできない。**最も手厚くテスト・レビューする**
- **汎用**: 複雑だが差にはならない、どこでも同じ部分。認証やメール配信など。**自分で作り込まず、既製品や外部サービスに任せる**
- **支援**: 単純で差にもならない部分。管理画面の一覧など。**軽く作る**

AI で開発する今、この 3 分類はそのまま「どこを人が手厚く見るか」の判断に効きます。

中核は人が伴走して厚く見る、汎用は買う、支援は AI に任せる。つまり手厚くテスト・レビューする先を選ぶことは、自分たちの価値の中心はどこかを見極める作業でもあります。
...
::: details 3 つの層の違いを、同じ機能で並べると
- **ユニット**: `<Button>` 単体、`formatPrice()` 単体。部品ひとつを孤立させて確かめる
- **結合**: フォームに入力 → 押す → 一覧が変わる、という部品どうしの協調を確かめる。通信は MSW（偽の API）で差し替え、jsdom（仮想の DOM）で動かすので、実ブラウザも本物のサーバも使わない。だから速くて安定する
- **E2E**: 本物のブラウザ（Playwright）で、本物のサーバまで通して「ログイン → 購入」まで動かす
:::

下の層ほど**速くて安くて、原因の特定が簡単**。上の層ほど**本物に近くて確信が強いが、遅くて壊れやすい**。

では、それぞれを何割ずつ持つべきか。この配分には、形の名前が付いています。
...
**安くて速い層を土台にたくさん、高くて遅い層は頂点に少しだけ**。E2E は遅く、壊れやすく、原因も遠いことを踏まえれば、自然な結論です。サーバーサイドでは、いまもこれが基本形です。レイヤーの数や名前は文脈で変わり、ユニット／サービス／UI と分けることもあります。

### テストトロフィー

React のようなコンポーネントベースのフロントエンドでは、ピラミッドを変形させた**テストトロフィー**という配分が広く支持されています（Kent C. Dodds が提唱）。優勝カップの形に 4 層を当てはめたものです。
...
- **最厚は「結合」**: フロントの結合テストは、フォームや検索のような **1 つの機能（複数の部品・状態・通信をまとめたもの）を、ユーザーの操作の目線で動かす**テスト。通信は偽物に差し替え、実ブラウザは使わないので、E2E より速くて安定する。フロントのバグの多くは部品ひとつずつではなく**部品どうしのつなぎ目**で起きるので、ここがコスパの山になる
- **土台は「静的解析」**: TypeScript と ESLint は、テストを 1 本も書かなくても全コードを常時チェックする。最安の層として土台に数える

### アンチパターン

配分が崩れた状態には、名前の付いた典型的な形があります。

- **逆ピラミッド（アイスクリームコーン）**: E2E ばかり大量。実行に時間がかかり、毎日どれかが不安定に落ち、誰も結果を信じなくなる
- **砂時計**: ユニットと E2E はあるのに中間が無い。部品は正しいのに、つなぎ目のバグが E2E まで素通りし、原因の特定に時間が溶ける

形は絶対のルールではなく目安です。大事なのは「安く速い層を厚く、遅く壊れやすい層は薄く」という重心です。

## AI が量産できる今

AI でテストは、E2E も含めて書きやすくなりました。でも配分の考え方は変わりません。

遅さ・壊れやすさ・保守という制約は残るので、結合を厚く、E2E は重要フローに絞る、という重心は同じです。

変わるのは人の役割です。

「何本書いたか」はもう指標になりません。**どこに・どんな質のテストを置くかを決め、量産された山から取捨する**のが、人に残る仕事です。
...
止めどきは、この終了基準の話です。目指すのは 100% ではありません。

カバレッジが 100% でもバグは残るからです。十分かどうかは、たとえばこんな状態を目安にします。
...
- 厚くする対象は 2 つの見方、機能ごとと層ごと
- AI が量産できる今も配分の考え方は変わらず、選ぶのが人の仕事
