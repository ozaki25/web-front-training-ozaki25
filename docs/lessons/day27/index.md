...
- 楽観的 UI 更新の考え方を理解する
- `useOptimistic` で即座にフィードバックを返せるようになる
- `useTransition` で優先度の低い更新を遅延できるようになる

## 楽観的 UI 更新とは

Day 26 でフォーム送信を学びました。API にデータを送信して結果を待つ間、ユーザーは「送信中...」と表示されて待つことになります。

しかし、SNS の「いいね」ボタンを想像してみてください。ボタンを押した瞬間にいいね数が増え、裏で API 通信が行われます。失敗した場合だけ元に戻します。
...
`useTransition` の典型的な使用例をもう1つ見てみましょう。
