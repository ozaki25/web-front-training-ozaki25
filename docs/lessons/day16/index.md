...
DOM の構造をリアルタイムで確認・編集できます。要素のスタイルもここで確認・変更できます。

- 要素を右クリック →「検証」で、その要素の位置に直接ジャンプ
- Styles パネルで適用されている CSS を確認
- どの CSS ルールが優先されているか（取り消し線が引かれているか）も確認できる

### Network タブ

Day 14 で触れた HTTP リクエストとレスポンスの詳細を確認できます。

- リクエスト一覧が時系列で表示される
- 各リクエストのヘッダー、レスポンスボディ、タイミングを確認
- ステータスコードで絞り込み（例: エラーだけ表示）
- 「Preserve log」にチェックを入れると、ページ遷移してもログが消えない
...
1. Sources タブを開く
2. 左側のファイル一覧からデバッグしたいファイルを開く
3. 行番号をクリックすると**ブレークポイント**（停止点）が設定される
4. ページを更新すると、その行で実行が一時停止する
5. 右側のパネルで変数の値を確認できる
6. 「Step Over」（次の行へ進む）や「Step Into」（関数の中に入る）で 1 行ずつ実行
...
> **本番コードに `console.log` を残さない**: デバッグ用の `console.log` は開発中だけ使い、本番環境にデプロイするコードには残さないようにしましょう。ESLint などのツール（後の Day で学びます）を使うと、`console.log` の消し忘れを自動で検出できます。
