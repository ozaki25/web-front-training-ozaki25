...
Biome や OXC はパーサ自体を Rust で書き直しており、パース速度も桁違いに上がっています。ただし統合（パーサ共有）と高速化（Rust 化）は独立した話です。JavaScript で書いてもパーサ共有は実現できますが、「ゼロから作り直すなら速度も取ろう」という判断で両方が同時に起きています。
:::
