# Day 25: ツールがコードを読む仕組み — AST という共通言語
...
やることは検査・変換・整形・削除とバラバラなのに、どのツールもコードの**意味**を理解しているように見えます。これらのツールは共通して **AST** という仕組みでコードを読んでいます。

## AST — コードを「木」として読む

ツールにとって、コードはただの文字列ではありません。

解析（パース）すると、**入れ子の構造を持った木**に変換できます。これを **AST**（Abstract Syntax Tree、抽象構文木）と呼びます。
...
全部読む必要はありません。`"type": "VariableDeclaration"` や `"name": "price"` が目に入れば十分です。

コードの各部分が `type` と値のペアで構造化されていて、ツールはこの構造をたどって処理しています。
...
今、この無駄を解消するツールが生まれています。複数のツールが 1 つのパーサを共有し、1 回のパースで検査も整形もやる設計です。
...
::: info 補足: 統合と同時に高速化も進んでいる
Biome や OXC はパーサ自体を Rust で書き直しており、パース速度も桁違いに上がっています。

ただし統合（パーサ共有）と高速化（Rust 化）は独立した話です。JavaScript で書いてもパーサ共有は実現できますが、「ゼロから作り直すなら速度も取ろう」という判断で両方が同時に起きています。
:::
...
- ツール名は世代交代しても AST は変わらず、パーサ共有で統合が進んでいる
