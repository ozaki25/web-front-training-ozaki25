...
Day 8 で学んだ Flexbox は「1 方向（横または縦）」のレイアウトが得意でした。CSS Grid は**2 方向**（横と縦の両方）を同時に制御できるレイアウトモデルです。
...
この 1 行で「最小 300px のカードを隙間なく敷き詰め、画面幅に応じて列数が変わる」というレスポンシブなレイアウトが実現できます。メディアクエリ（Day 10 で学びます）を使わなくても柔軟なレイアウトが作れます。
...
**次のレッスン**: [Day 10: レスポンシブデザインと CSS の課題](/lessons/day10/)
