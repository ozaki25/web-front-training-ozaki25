...
テキストと枠線の間にスペースが生まれ、**箱自体のサイズが広がりました**。padding は要素の内側に効くので、要素自身の大きさを変えます。
...
**箱自体のサイズは変わらず**、上の箱との距離が広がりました。margin は要素の外側に効くので、要素自身ではなく配置が変わります。

ここで注目してほしいのは**主語の違い**です。padding は「要素**自身が**内側を広げる」もの。margin は「**親が**子をどう配置するか決める」ものです。同じ余白でも、誰が決めるかが違います。この視点は、後で出てくる「コンポーネントに margin を持たせない」設計につながります。
...
- margin は一方向（`margin-top` と `margin-left` だけなど）に統一すると相殺を避けられます
