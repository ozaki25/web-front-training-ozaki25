...
つまりブラウザ側で見るぶんには、`NEXT_PUBLIC_` はさきほどの `const ENABLED = false` と同じで、フラグの置き場所になりません。
...
運用のフラグは、アクセスが集中したときに重い機能だけ止めて、サービス全体が落ちるのを防ぐために使います。落ち着いたら消すものが多いですが、いつでも止められる備えとして残すこともあります。

権限のフラグは、有料プランの人にだけ機能を出すような出し分けです。その機能がある限り消えません。
