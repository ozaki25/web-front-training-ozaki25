...
Next.js はページを先に作ろうとします。そのとき、コードが「アクセス時に確定する情報」を読もうとしても、ビルド時にはまだ確定していないので先に作れません。だから動的になります。

- アクセス時に確定する情報（ログイン中のユーザー、URL の検索ワードなど）を読む → 先に作れないので動的。アクセスごとに組み立てる
- 読まない → 先に作って保存できるので静的

```mermaid
flowchart TD
    P["ページを先に作ろうとする"] --> Q{"アクセス時に確定する<br/>情報を読む？"}
    Q -->|読む| D["動的：アクセスごとに作る"]
    Q -->|読まない| S["静的：先に作って保存"]
...
コードの上でその「アクセス時に確定する情報」にあたるのが、`cookies()`（ログイン情報）、`headers()`、`searchParams`（URL の検索ワード）です。これらを読んでいれば動的、読んでいなければ静的、と判定されます。
