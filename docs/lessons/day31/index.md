...
`fetch` の `cache` や `revalidate` の指定は、基本的には取ってきたデータをどれだけ新しく保つかを決めるものです。ここは静的か動的かの判定と混同しやすいところですが、別の話です。ただし例外がひとつあり、`cache: "no-store"`（毎回サーバーから取り直す指定）を書いたときだけは、そのページが動的になります。`connection()` と同じく「毎回作り直す」の合図になるからです。
