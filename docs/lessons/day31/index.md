...
`fetch` に `cache: "no-store"` を付けると、そのページは必ず動的になります（毎回サーバーで組み立て直す）。在庫やレートのように毎回最新が要る、静的化させたくないページで使います。
...
従来モデルでは、`fetch` は指定がなければキャッシュされません（＝そのページは動的）。なので `no-store` を足しても挙動は変わらないことが多く、「動的にする」という意図を明示する意味合いが中心です。
:::
