...
大事なのは、静的にするか動的にするかを**開発者が指定するのではない**ことです。Next.js がページのコードを上から実行し、何を使っているかを見て**自動で判定**します。

分かれ目は、ページの中身が**見る人やタイミングで変わるか**どうかです。具体的にはこういう違いです。

- **変わる**なら動的: ログイン中のユーザー名、その人のカート、検索ワードで変わる検索結果、在庫数や現在時刻
- **変わらない**なら静的: 会社概要やブログ記事や利用規約のように、誰がいつ見ても同じページ

コードの上では、`cookies()` でログイン情報を読む、`headers()` を見る、URL の `searchParams` を使う、といった「人やアクセスごとに変わるもの」に触れていれば動的、触れていなければ静的、と Next.js が判定します。

```mermaid
flowchart TD
    P["ページを実行"] --> Q{"見る人やタイミングで<br/>中身が変わる？"}
    Q -->|変わる| D["動的：毎回組み立てる"]
    Q -->|変わらない| S["静的：保存して使い回す"]
    style D fill:#fee2e2,color:#1e293b,stroke:#dc2626
    style S fill:#dcfce7,color:#1e293b,stroke:#16a34a
```

さきほどの `/about` は `cookies()` のようなものを使っていないので、Next.js は自動的に静的と判定します。
...
export default async function DashboardPage() {
  await connection(); // ここから先はリクエストごとに実行する
...
`connection()` は「ここから先はビルド時に先取りせず、リクエストが来てから実行する」という合図です。これでそのページは動的になり、毎回サーバーで組み立て直されます。
