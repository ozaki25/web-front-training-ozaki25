...
カギは、ビルドの時点では**まだ誰もそのページを見に来ていない**ことです。ログインしている人もいなければ、URL に付く検索ワードも決まっていません。

Next.js はページを先に作ろうとします。そのとき、コードが「アクセスが来て初めて分かる情報」を読もうとしたら、ビルド時にはその情報が無いので先に作れません。だから動的になります。

- アクセスが来て初めて分かる情報（ログイン中のユーザー、URL の検索ワードなど）を読む → 先に作れないので動的。アクセスごとに組み立てる
- 読まない → 先に作って保存できるので静的

```mermaid
flowchart TD
    P["ページを先に作ろうとする"] --> Q{"アクセスが来て初めて<br/>分かる情報を読む？"}
    Q -->|読む| D["動的：アクセスごとに作る"]
    Q -->|読まない| S["静的：先に作って保存"]
...
コードの上でその「アクセスが来て初めて分かる情報」にあたるのが、`cookies()`（ログイン情報）、`headers()`、`searchParams`（URL の検索ワード）です。これらを読んでいれば動的、読んでいなければ静的、と判定されます。

さきほどの `/about` は、こうした情報を読んでいないので、先に作れるとみなされて静的になります。
