...
`revalidatePath` には第 2 引数があり、保存済み HTML を**どこまで作り直すか**を決めます。
...
全ページ共通の部分を変えたときは `"layout"` を使います。たとえばフッターの会社情報のように、レイアウトに置いて全ページの HTML に焼き込まれているものを変えたなら、1 ページだけでなく配下の全ページを作り直す必要があります。

```ts
revalidatePath("/", "layout"); // ルートレイアウト配下すべての HTML を作り直す
```

`"page"` で 1 ページだけ作り直しても、ほかのページの保存済み HTML は古いままです。共通部分を変えたなら `"layout"` で範囲を広げます。
