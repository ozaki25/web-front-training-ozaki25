...
どれも同じレジストリ（[npmjs.com](https://www.npmjs.com/)）からパッケージをダウンロードしますが、依存関係の解決方法が異なります。npm と yarn は依存をフラットに展開するのに対し、pnpm はシンボリックリンクを使って厳密に分離します。そのため、同じ package.json でもツールによって node_modules の中身の構造が変わります。
...
`^` が付いているとバージョンに範囲があるため、インストールするタイミングによって微妙に違うバージョンが入る可能性があります。これを防ぐのがロックファイルです。ロックファイルに「実際にインストールされた正確なバージョン」が記録されているので、チーム全員が同じバージョンを使えます。
...
- `^` や `~` はバージョンの範囲指定。ロックファイルで実際のバージョンを固定する
- npm / yarn / pnpm は依存解決の方法やロックファイルが異なる。ロックファイル名でどのツールを使っているかわかる
