...
仕組みはシンプルです。CSS カスタムプロパティは、**より具体的なセレクタで同じ変数名に別の値を定義する（上書きする）** ことができます。`:root` で定義した値を、`[data-theme="dark"]` の側がまるごと上書きしているのです。
...
この JSON 形式で定義したトークンを、**Style Dictionary や Tokens Studio といったツール**が、CSS カスタムプロパティ・Tailwind の設定・iOS / Android 向けのコードに自動変換します。

手作業で各プラットフォーム向けに書き写すのではなく、**1 つの定義から複数のプラットフォームのコードを生成する**という流れです。デザイナーがトークンを更新すれば、変換ツールが走り、Web もアプリも同じ値に揃います。

```mermaid
flowchart LR
  A["デザイントークン<br>（JSON）"] -->|"Style Dictionary<br>などで変換"| B["CSS カスタムプロパティ"]
  A -->|変換| C["Tailwind @theme"]
  A -->|変換| D["iOS / Android"]
...
設定の書き方まで覚える必要はありません。「トークンの JSON を各プラットフォーム用に変換する定番ツールがある」という引き出しがあれば、チームでこの仕組みの話が出たときについていけます。

## まとめ

- デザイントークン = 色・サイズ・余白に名前をつけて一元管理する仕組み
- `--名前` / `var()` の CSS カスタムプロパティがブラウザ標準の実現手段
- プリミティブ（色そのもの）とセマンティック（役割）の 2 層で設計する
- ダークモードはトークンの値を切り替えるだけ
- Tailwind v4 の `@theme` でトークン定義を CSS 内に完結
- デザイナーと同じトークン名を使えば会話がスムーズになる
- JSON のトークンを Style Dictionary などが各プラットフォーム用コードに自動変換する
