...
それでも `cache: "no-store"`（キャッシュしないという明示）を付けたコードはよく見かけます。`no-store` を付けると、そのページは**必ず動的レンダリングになります**。
...
動的レンダリング自体は悪ではありません。ログイン後のダッシュボード、在庫やレートのようなリアルタイムの数値など、**毎回最新を作るのが正しいページはたくさんあります**。動的にするかは「このページは本当に毎回最新が要るか」で決めます。

ただし従来モデルでは、`fetch` の既定がすでに「キャッシュしない＝動的」です。そのため「常に最新にしたい」という理由で付ける `no-store` は、動きを変えずにコードを増やしているだけのことが多くあります。さらに、静的で十分なページにまで付けると、静的化できたはずのページを動的に倒してしまい、速さを手放すことになります。
...
なお、ここで保存しているのは「組み立てた HTML」です。その手前の「取得したデータ」の保存や、ブラウザ側の保存は、段階の違う別のキャッシュです。

このレッスンでは HTML の箱に絞っています。
