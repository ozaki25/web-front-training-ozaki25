...
Next.js は、`revalidate` や `force-cache` でキャッシュ指定があり、`cookies()` のようにユーザーごとに変わる要素を使っていないページを**自動で静的化**しようとします。`fetch` は指定がなければキャッシュされないので、指定なしの `fetch` があるだけでそのページは動的になります。

::: details no-store で明示的に動的にする
`fetch` に `cache: "no-store"` を付けると、そのページは必ず動的になり、毎回サーバーで組み立て直します。在庫やレートのように毎回最新が要る、静的化させたくないページで使います。
...
従来モデルでは、`fetch` は指定がなければキャッシュされず、そのページは動的になります。なので `no-store` を足しても挙動は変わらないことが多く、「動的にする」という意図を明示する意味合いが中心です。
:::
...
| 上記に `revalidatePath` を追加 | 静的のまま、変更時に作り直す | 速い | 変えた後に最新へ |
...
- デフォルトで動的。静的にするには `fetch` にキャッシュ指定が要る
