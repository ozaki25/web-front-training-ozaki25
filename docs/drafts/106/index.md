# テスト計画 — 限られたテスト工数をどこに配るか

## 今日のゴール

- 全部はテストできないので、限られた工数を配分すると知る
- 「機能ごと（リスク）」と「層ごと（ピラミッド/トロフィー）」の 2 軸で配ると知る
...
「**全数テストは不可能**」は、テストの原則のひとつです。だからプロも「全部」ではなく「どこに力を入れるか」を決めます。この**配分を決めるのがテスト計画**です。配る軸は 2 つあります。

- **機能で配る**: どの機能を厚く／薄くするか（リスク）
- **層で配る**: どの種類のテストをどの割合で持つか（ピラミッド/トロフィー）
...
AI がテストを量産できる今、**書くコストはほぼ消えました**。でも、それで制約が無くなったわけではありません。制約が移っただけです。
...
- 配る軸は 2 つ、機能ごと（リスク）と層ごと（ピラミッド/トロフィー）
- AI が量産できる今、制約は「書く量」から「遅さ・壊れやすさ・信頼」へ移り、選ぶのが人の仕事
