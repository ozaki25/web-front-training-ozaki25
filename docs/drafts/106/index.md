...
「**全数テストは不可能**」は、テストの原則のひとつです。だからプロも「全部」ではなく「どこに力を入れるか」を決めます。この**力の配りどころを決めるのがテスト計画**です。
...
この 2 軸の図を**リスクマトリクス**と呼びます（リスクを大小で見積もる定性的なやり方。数値で出すなら、リスクレベルは可能性と影響の掛け算です）。読み方はシンプルです。
...
- **確認**: テストの山が、リスクの高いところに集中しているかを見る。低リスクに大量で高リスクが手薄なら、並べ替える
