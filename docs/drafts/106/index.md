...
力の入れどころは、リスクで決めます。この進め方を**リスクベースドテスト**と呼びます。リスクの大きさ、つまりリスクレベルは、2 つの掛け合わせで決まります。
...
この 2 軸の図を**リスクマトリクス**と呼びます。大小で見積もる定性的なやり方です。数値で出すなら、リスクレベルは可能性と影響の掛け算になります。読み方はシンプルです。
...
AI で開発する今、この 3 分類はそのまま「どこを人が手厚く見るか」の判断に効きます。中核は人が伴走して厚く見る、汎用は買う、支援は AI に任せる。つまり手厚くテスト・レビューする先を選ぶことは、自分たちの価値の中心はどこかを見極める作業でもあります。
...
配分が崩れた状態は**アンチパターン**と呼ばれ、それぞれ名前が付いています。
...
AI でテストは、E2E も含めて書きやすくなりました。でも配分の考え方は変わりません。遅さ・壊れやすさ・保守という制約は残るので、結合を厚く、E2E は重要フローに絞る、という重心は同じです。

変わるのは人の役割です。「何本書いたか」はもう指標になりません。**どこに・どんな質のテストを置くかを決め、量産された山から取捨する**のが、人に残る仕事です。
...
止めどきは、この終了基準の話です。目指すのは 100% ではありません。カバレッジが 100% でもバグは残るからです。十分かどうかは、たとえばこんな状態を目安にします。
