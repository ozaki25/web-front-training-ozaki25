...
- `no-store` が静的化をやめさせること、再検証で作り直せることを知る
...
Next.js は、ページが動的な要素を使っていなければ**自動で静的化**しようとします。開発者が「これは静的」と細かく指定しなくても、可能なら保存される側に倒れます。

## no-store が静的化をやめさせる

ここで、よくある事故につながる挙動があります。データ取得で `cache: "no-store"` を付けると、そのページは**静的化をやめて、必ず動的レンダリングになります**。
...
AI に「常に最新にして」と頼むと、`no-store` をあちこちに付けたコードを書いてくることがあります。これをやると、本来は静的にできたページまで全部動的になり、**静的化による速さを丸ごと失います**。鮮度は得られますが、その代償は大きい。「本当に毎回最新でないと困るのか」を一度問う価値があります。

| 指定 | レンダリング | 速さ | 鮮度 |
|------|------------|------|------|
| 指定なし（既定） | 可能なら静的 | 速い | ビルド時点で固定 |
...
- `no-store` は静的化をやめさせ、全ページ動的に倒すので乱用に注意
