...
つまり「失敗したら戻す」ためのコードを自分で書いていないのに、ロールバックが成立しています。楽観的な値を別の変数として持ち、本当の値と混ぜないから、確定と巻き戻しの管理をフックに任せられます。なお `addOptimisticCount` はこの例のようにフォームの action（またはトランジションと呼ばれる更新）の中で呼ぶ決まりになっています。
