...
- React の useOptimistic でこのパターンを標準の書き方にできると知る
- 楽観的か悲観的かは失敗したときの被害の大きさで選ぶと知る

## いいねの数字が一瞬で増える理由

SNS で「いいね」を押すと、数字はその瞬間に増えます。投稿ボタンを押すと、自分の投稿がすぐ一覧の先頭に現れます。当たり前の光景ですが、いいねが本当に記録されるのはサーバーの中なので、通信が終わる前に画面が変わっているのはよく考えると不思議です。

通信には往復の時間がかかります。ブラウザからリクエストがサーバーに届き、サーバーが処理して、応答が返ってくるまで、速くても数十ミリ秒、モバイル回線なら 1 秒近くかかることもあります。人は押してから 0.1 秒を超えたあたりで遅れを感じ始めると言われます。
...
楽観的更新（optimistic update）は、「どうせ成功するだろう」と楽観して、サーバーの応答を待たずに画面を先に更新する手法です。本当のリクエストは裏で送っておき、結果に応じてつじつまを合わせます。
...
注意したいのは、これが**見せ方の技術**だという点です。データの正しさを決めるのはあくまでサーバーで、画面が先に変わってもサーバーのデータが変わったわけではありません。だから失敗したときに元へ戻す処理まで含めて、初めて成立します。
...
つまり「失敗したら戻す」ためのコードを自分で書いていないのに、ロールバックが成立しています。楽観的な値を別の変数として持ち、本当の値と混ぜないことで、確定と巻き戻しの管理をフックに任せる仕組みです。なお `addOptimisticCount` はこの例のようにフォームの action（またはトランジションと呼ばれる更新）の中で呼ぶ決まりになっています。

## 悲観的更新との使い分け

楽観的更新の対になるのが**悲観的更新**（pessimistic update）です。「失敗するかもしれない」と悲観して、サーバーの成功応答を確認してから画面を更新します。押してから変わるまでの待ちは発生しますが、画面に出た結果は必ずサーバーで確定しています。
...
選ぶ基準は速さではなく、**失敗したときの被害の大きさ**です。いいねが 1 回消えても誰も困りませんが、「注文が完了しました」と見せた注文が実は失敗していたら大事故です。被害が大きい操作では、待ち時間を隠すのではなく、ボタンを無効化してスピナーを出し、「処理中」だと正直に見せるほうが正しい設計になります。

AI に実装を任せるときも、この語彙が効きます。「いいねは楽観的更新にして、失敗時はロールバックとエラー表示を入れて」「決済の確定は悲観的更新で、処理中はボタンを無効化して」のように、欲しい挙動を一言で指定できます。

## 失敗時の設計

楽観的更新でいちばん危ないのは、失敗の扱いを省略することです。画面では成功に見えているぶん、失敗を伝え損ねると「押したのに実は反映されていなかった」という、利用者が気づきようのない事故になります。楽観的更新を入れるときは、次の 3 点をセットで設計します。
...
逆に言うと、この 3 点を確認すれば、楽観的更新を含むコードの良し悪しを評価できます。画面を先に変えるコードを見つけたら、「失敗したらどうなるか」を尋ねる。それがこのパターンをレビューするときの定番の視点です。
