...
よく見ると、「失敗したら戻す」ためのコードはどこにも書いていません。楽観的な値を本当の値とは別の変数で持つので、確定も巻き戻しも useOptimistic に任せられます。

なお `addOptimisticCount` は、この例のようにフォームの action（またはトランジションと呼ばれる更新）の中で呼ぶ決まりになっています。
...
- 被害が大きい操作では、待ち時間を隠すのではなく、ボタンを無効化してスピナー（くるくる回る処理中の表示）を出し、「処理中」だと正直に見せるほうが正しい設計になる
