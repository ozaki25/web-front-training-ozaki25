...
`margin-bottom` だけ、または `margin-top` だけに統一すれば、上下から margin がぶつかることがないので、相殺は起きません。どちらでもいいですが、`margin-bottom`（下方向）に統一するのが一般的です。上から下に読むページの流れに沿って「次の要素との距離」を指定するほうが直感的だからです。
