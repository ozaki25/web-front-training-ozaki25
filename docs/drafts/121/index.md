...
名前の付いた仕組みが 2 つ関わっています。

> **リテラル型**: `"primary"` のような、具体的な値そのものを表す型

> **型の拡大**（widening）: あとで書き換えられる場所に置かれたリテラル値を、リテラル型ではなく広い型（`"primary"` なら `string`、`1` なら `number`）で推論する TypeScript の挙動
...
- **`const color`**: 再代入できないので、リテラル型 `"red"` のまま保たれる
- **`let color`**: あとで `color = "blue"` と書き換えられるため、TypeScript は「別の文字列が入るかもしれない」と考えて `string` に広げる

見落としやすいのが 3 行目です。**オブジェクト自体を `const` で宣言しても、プロパティは広がります**。

- `const obj` が禁止するのは、`obj` への再代入だけ
- `obj.color = "blue"` という書き換えは禁止しない
- だから冒頭の `submitButton.variant` も `string` になっていた
...
- **`typeof SIZES`**: 値 `SIZES` の型を取り出す構文
- **`[number]`**: その配列の要素の型、という意味
- **`as const` がないと**: `SIZES` は `string[]` に広がり、導出される型もただの `string` になってしまう

「選択肢の一覧を値として持ち、そこからユニオン型を作る」という形は、実際のコードで頻繁に登場します。
...
かといって注釈を外すと、別の穴が開きます。

| 書き方 | キーの情報 | `Theme` への適合チェック |
|--------|-----------|------------------------|
| `: Theme` と注釈を付ける | 消える | される |
| 注釈を外す | 残る | されない（値に数値を混ぜても気づけない） |
...
> 「この配列は `as const` にして、そこからユニオン型を導出して」

> 「型注釈だと具体的なキーの情報が消えるから、`satisfies` に変えて」

このように、型の情報をどう保つかを具体的に指定できます。
