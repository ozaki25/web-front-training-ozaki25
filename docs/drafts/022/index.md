# key の正体 — React は一覧の「誰が誰か」をどう見分けるか
...
key を付け忘れるとコンソールに警告が出るため、「警告を消すためのおまじない」と思われがちです。AI に聞いても「一意な値を付けてください」と教えてくれますが、**なぜ一意でなければならないのか**まではコードから読み取れません。
...
この変化は「りんごが削除された」のか、「全部の内容が 1 つずつ繰り上がって最後が消えた」のか。**並びだけでは区別できません**。
...
key 1 が消えて、key 2 と 3 は残っている。つまり「りんごの削除」だと特定できます。残った 2 つは**作り直さずそのまま再利用**されます。
...
ここで、key に `index`（配列の何番目か）を使うとどうなるか。メモ欄付きのタスク一覧で考えます。
...
React から見ると「key=0 は削除後も存在する」ので、key=0 の状態（メモ欄の「至急」）はそのまま残ります。しかし key=0 の**表示内容**はタスク B に変わっている。結果、**タスク A に書いたはずのメモが、無関係なタスク B の行に引っ越して見える**のです。
...
一覧のためだけのものに見えた key は、実は「**これは同じものか、別物か**」を React に伝える汎用の名札でした。
