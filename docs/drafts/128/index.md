...
一度送ったメールを相手の受信箱から回収しているように見えますが、そうではありません。この時点で Gmail は**まだメールを送信していません**。設定に応じて数秒から 30 秒ほど送信を保留し、その間だけ「元に戻す」を見せています。押されたら、保留していた送信そのものを中止します。
...
**毎回出る確認は、読まれなくなります。** 削除のたびにダイアログが出ると、人は「OK を押せば進む」ことだけを学習し、文面を読まずに反射で OK を押すようになります。守るつもりで挟んだ確認が、慣れると単なる追加のワンクリックになります。

もうひとつ、より根本的な弱点があります。**誤操作の瞬間、本人は間違えているつもりがない**ことです。消すファイルを取り違えた人は、「正しいファイルを消している」と思いながらダイアログを見ます。だから迷わず OK を押します。間違いに気づくのは、実行された結果を見た後です。事前の確認は、そのタイミングでは間に合いません。
...
誤操作した本人は、実行前には自分を疑っていません。結果が画面に出て初めて「違う、これじゃない」と気づきます。Undo は、その**気づいた後**に救済が間に合う守り方です。
...
Gmail の送信取り消しがこの型です。画面では「送信しました」と表示しつつ、本当の処理は保留しておきます。「元に戻す」が押されたら、保留していた実行そのものをやめます。
...
削除の Undo でよく使われるのがソフトデリートです。「削除」と言いながら、実データは消しません。削除フラグを立てて、画面に出さなくするだけです。
...
利用者から見れば消えていますが、データは残っているので、いつでも戻せます。メールソフトやファイル共有サービスの「ゴミ箱」はこの型で、本当の削除はゴミ箱を空にしたときや、30 日のような保管期限が来たときにまとめて行います。

### 共通の構造

2 つの型は、やっていることは違っても構造は同じです。**画面上の出来事と、取り返しのつかない実行を切り離しています。** 画面では「送信した」「削除した」と見せながら、裏では取り消せない一線をまだ越えていません。Undo が押されるかもしれない時間だけ、その一線の手前で待っています。
...
たとえば GitHub でリポジトリを削除するときは、確認画面でリポジトリ名をそのまま入力させられます。この名前の入力は、わざと手間を増やして、反射で OK を押せなくする工夫です。読まずに押せるボタンと違い、対象の名前を打ち込むには、いま何を消そうとしているかを見るしかありません。

ここまでを並べると、守り方は手間の強さの階段になっています。

- 失敗してもすぐ戻せる操作: Undo で流れを止めない
- 戻せるが影響の広い操作: 確認ダイアログで一度止める
- 戻せず被害の大きい操作: 名前の入力のように手間を意図的に増やす
