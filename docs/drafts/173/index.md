...
**Cookie** や **localStorage** と呼ばれるものです。
...
ログインの印には Cookie がよく使われます。リクエストに自動で添えられる仕組みがあるからです。

下書きのように、サーバーへ送る必要がなくてブラウザの中だけで使うものには localStorage が向きます。

もう1つ、URL に書いておくやり方もあります。`?sort=price&page=2` の `?` 以降を**クエリパラメータ**と呼び、並び順やページ番号を毎回ここで伝えています。
...
| ブラウザ（Cookie、localStorage） | ログイン済みの印、下書き | ブラウザを閉じても残る。その端末の中だけ |
| URL（クエリパラメータ） | 並び順、ページ番号、絞り込み | その URL を開いている間。人にも送れる |
| どこにも置かない | 書きかけの文章、スクロール位置 | リロードすると消える |
