...
この `"use client"` を消すとエラーになるファイルもあれば、付いていないファイルもある。この違いは、現在の React / Next.js の根幹である**コンポーネントの 2 種類の区別**に関わっています。
...
ページを見渡すと、ヘッダーのロゴや記事の本文、商品の説明文、フッターなど、**ユーザーが操作しない部分が大半**です。操作しない部品のコードをブラウザに送って実行させるのは、ただの無駄でした。
...
- コンポーネントは SC（サーバーだけ）と CC（ブラウザでも）の 2 種類で、デフォルトは SC
- 操作しない部品が大半だから、JS を送らない SC が標準になった
- "use client" は境界宣言で、import 先まで芋づる式にブラウザ行きになる
- 境界は葉っぱに引き、上位ファイルの "use client" は要注意
