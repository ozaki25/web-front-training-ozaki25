...
「速さ」と「新しさ」は引っ張り合いの関係です。新モデルでは、この綱引きのさじ加減を `"use cache"` でコードに宣言します。
...
宣言が無ければキャッシュされません。どこを使い回すかは開発者が明示的に選びます。
...
ミリ秒の設定値ではなく、**業務的な鮮度の宣言**です。「在庫は秒単位で正確であってほしい」「会社概要は日単位でいい」という要件がそのままコードになります。期限が切れたら、次のアクセスで作り直されます。
...
`revalidateTag` は、第 2 引数に `cacheLife` のプロファイル（`"max"` など）を渡します。古い結果を即座に返しつつ裏で作り直す動き（stale-while-revalidate）になります。不特定多数が見るページで「待たせない」を優先するときに向きます。
