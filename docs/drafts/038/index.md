...
アクセスのたびにデータ元へ問い合わせてページを組み立てていたら、人気のページほど遅くなります。そこで使われるのが**キャッシュ**、つまり「一度作った結果を保存して使い回す」仕組みです。

ただしキャッシュにはつきものの問題があります。**使い回している間にデータが変わったら、古い表示が出続ける**。
...
取得の手段も問いません。`fetch` でもデータベースへの問い合わせでも、同じ `"use cache"` で書けます（従来モデルで `fetch` 以外に必要だった `unstable_cache` は不要です）。
...
時間ベースには弱点があります。`cacheLife("hours")` の商品カタログで価格を変更したら、**最悪数時間、古い価格が出続けます**。

かといって鮮度を短くすれば、キャッシュの意味が薄れます。
