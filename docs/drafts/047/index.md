...
このテストでもカバレッジは上がります。**実行された**ことと**正しさを確かめた**ことは別物で、`expect`（期待値の照合）が弱ければ、カバレッジ 100% のコードにもバグは平気で住めます。数を増やすだけの照合の薄いテストは、数字が上がっても意味がないので、**数字より expect の中身**を見てください。
...
テストコードを自分で量産する時代は終わりつつありますが、**「何を試すべきか」を見極める力**の価値は、むしろ上がっています。
