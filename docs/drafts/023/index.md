...
この罠を踏むと、「コードは正しそうなのに色だけ付かない」状況を作ります。**クラス名は必ず完全な形でコードに書く**。原因を知っていれば、「クラス名を組み立てているから効かないのでは」と一目で疑えます。
...
::: tip 古い設定ファイルを見かけたら
以前のバージョンでは `tailwind.config.js` という JavaScript の設定ファイルが必須でした。古い記事や古いスタイルの設定を見かけたら、「いまは CSS に書く方式」と思い出してください。
:::
...
- Tailwind は 1 クラスが 1 スタイルのユーティリティファーストで、値は決められた目盛りから選ぶ
- ビルド時にソースをスキャンし、書かれているクラスの分だけ CSS を生成する
- だから実行時に組み立てたクラス名は効かず、クラス名は完全な形で書く
