...
このコードは、一見すると正当な用途です。「ログイン前のページに戻す」のは ユーザーにとって便利な機能です。

ところが、攻撃者が次の URL でリンクを作ると、危険になります。

```
https://example.com/login?next=https://attacker.com/phishing
```

ユーザーが信頼しているサイト（example.com）でログインしているのに、成功後に詐欺サイト（attacker.com）へ飛んでいきます。ユーザーは「example.com は信頼できるサイトなのだから、このリダイレクト先も安全だろう」と思い、attacker.com で再度パスワードを入力してしまう。これが **Open Redirect** による **フィッシング** です。
...
ただし、クライアント側の検証だけでは不足です。攻撃者は JavaScript を読んで許可ルールを逆算し、その範囲内で malicious URL を作ります。サーバーでの検証こそが唯一の信頼できる防御です。
...
すべて同じ原則。**リダイレクト先が外部入力に依存する場合は、サーバーで同一オリジンか ホワイトリストか確認する**。
...
- クライアント側の検証だけでは不十分。サーバーサイドの検証が信頼できる防御
