...
ところが実はこの文字列、**鍵もパスワードもなしで、誰でも一瞬で中身を読めます**。
...
| ③ 署名 | 改ざんを見破るための値。これだけは鍵がないと作れない |

ヘッダーとペイロードは、JSON を **Base64URL** という方式で文字列に変えたものです。

> **Base64URL**: どんなデータも、URL に載せても壊れない文字だけで表すための変換。暗号化ではない

- 変換のルールは全部公開されていて、元に戻すのに鍵は要らない
- ローマ字で書かれた日本語を読み戻すのと同じで、ルールを知っていれば誰でも戻せる

## atob で読めるペイロード

実際に読むのは簡単で、ブラウザの DevTools のコンソールに次のコードを貼るだけです。ライブラリも通信も要りません。
...
- `atob()`: ブラウザに組み込まれている Base64 デコード関数
- 途中の `replace`: Base64URL は `+` と `/` の代わりに `-` と `_` を使うので、その 2 文字を標準の Base64 に戻している
...
| `sub` | ユーザー ID |
| `role` | 権限 |
| `exp` | 有効期限。1970 年からの経過秒で、この値は 2027-01-01 |

ここまで鍵は一度も出てきていません。**JWT のペイロードは、トークンを手にした人なら誰でも読めます**。

実務では jwt.io のようなデコードツールに貼って確認する人も多いですが、本物のトークンを外部サイトに貼るのは避けたほうが安全です。手元の `atob()` で足ります。

## 署名の役割は改ざん検知

3 つ目の署名の役割は、**中身を隠すことではなく、書き換えを見破ること**です。署名は、ヘッダーとペイロードを材料にして、**サーバーだけが知っている鍵**で計算した値です。

- **HS256**: 代表的な方式。サーバーの秘密の鍵を混ぜた HMAC-SHA256 という計算で署名を作る
- **RS256**: 秘密鍵で署名を作り、対応する公開鍵で検証する方式

どちらも共通するのは「**正しい署名は鍵がないと計算できない**」ことです。

攻撃者が自分のトークンの `role` を `admin` に書き換えたとします。

- 中身は読めるので、書き換え自体は簡単
- でも、書き換えた後の内容に対応する正しい署名は、鍵を知らないと作れない
- サーバーは受け取るたびに署名を検証するので、そこで不一致が見つかる
...
> 署名が保証するのは「サーバーが発行した後、1 文字も書き換えられていない」ことだけ。「他人に見せない」効果はない
...
この基準は、AI にログイン機能を作らせるときの語彙にもなります。「**JWT のペイロードには見られて困る情報を入れず、ユーザー ID と権限と有効期限だけにして**」と一言添えれば、事故の芽をひとつ潰せます。

チームで「JWT って中身は暗号化されているんですか」という話になったときも、「エンコードだけなので誰でも読めます。ただし署名があるので改ざんは検知できます」と正確に答えられます。
