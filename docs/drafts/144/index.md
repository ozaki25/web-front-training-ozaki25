...
> **Base64URL** = どんなデータも、URL に載せて壊れない文字だけで表すための変換。暗号化ではない
...
- **RS256**: 署名を秘密鍵で作り、検証は公開鍵で行う方式もある

どちらも共通するのは「**正しい署名は鍵が無いと計算できない**」ことです。
...
この基準は、AI に指示するときの語彙にもなります。

- ログイン機能を作らせるとき: 「**JWT のペイロードには見られて困る情報を入れないで。ユーザー ID と権限と有効期限だけにして。秘密の情報はサーバー側で管理して**」と言えれば、事故の芽をひとつ潰せる
- レビューで「この JWT、中身は暗号化されてますか」と聞かれたとき: 「エンコードだけなので誰でも読めます。ただし署名があるので改ざんは検知できます」と正確に答えられる
