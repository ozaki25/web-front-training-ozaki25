...
- **組み合わせの理解**: `useCallback` は `React.memo` されたコンポーネントに渡して初めて効く、のような関係を正しく押さえる必要がある
...
段階的に導入したい場合は、設定を `{ compilationMode: "annotation" }` にすると、先頭に `"use memo"` という目印（ディレクティブ）を書いたファイルだけがコンパイル対象になります。
