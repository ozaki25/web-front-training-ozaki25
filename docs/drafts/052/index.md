...
あなたが銀行にログインしたまま、まったく関係のない別のサイトを開いたとします。そのサイトを開いた瞬間、あなたのブラウザから銀行へ 10 万円の送金リクエストが飛びます。

あなたは送金ボタンを押していません。それどころか、開いたのは銀行とは無関係のサイトです。

これが **CSRF**（Cross-Site Request Forgery、サイトをまたいだリクエストの偽造）です。別のサイトから、ログイン中の本人になりすまして操作を送り込む攻撃を指します。

パスワードは盗まれていません。盗まれたのは操作そのものです。

銀行のサーバーから見ると、本人がやった送金と見分けがつきません。

## なぜ起きるのか

原因は、**Cookie が自動で送られる**ことです。

ログイン状態は Cookie で保たれています。Cookie はブラウザが預かっている合鍵のようなもので、リクエストの宛先が銀行であれば、ブラウザが自動で付けてくれます。

問題は、この自動添付が **どのサイトから送られたかを問わない** ことです。宛先が銀行でありさえすれば、まったく別のサイトに置かれたフォームからの送信でも、ブラウザは律儀に銀行の Cookie を付けます。

だから攻撃者は、こんなフォームを自分のサイトに置いておくだけで済みます。
...
ユーザーがこのページを開くと、スクリプトがフォームを自動で送信します。ブラウザは銀行の Cookie を添えて送るので、銀行には「ログイン済み本人からの送金依頼」が届きます。
...
ブラウザには、別サイトのデータを JavaScript から読ませない防御（同一オリジンポリシー）があります。しかし送信そのものは昔から別サイト宛てに可能で、そこに Cookie が付くのが CSRF の急所です。

読み取りは防がれているのに、操作は通ってしまう隙間です。
