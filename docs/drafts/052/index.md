...
すると、その別サイトを開いただけで、あなたの口座から勝手に送金が実行されてしまう。ボタンを押した覚えもないのにです。
...
カギは、ログイン状態を保っている **Cookie が自動で送られる** ことです。
...
ユーザーがこのページを開くと、スクリプトがフォームを自動で送信します。フォームの送信先は銀行なので、ブラウザは銀行の Cookie を付けて送ってしまいます。
:::
