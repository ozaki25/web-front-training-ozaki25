...
サーバー側から見ると、正規の操作と区別がつきません。パスワードは盗まれていないのに、操作だけが偽造される。
...
ブラウザにはもともと「別サイトのデータを JavaScript から**読ませない**」防御（同一オリジンポリシー）があります。しかし**送信そのもの**（フォームの POST）は昔から別サイト宛てに可能で、そこに Cookie が付くことが CSRF の急所。
...
ただし、自前で API（Route Handler）を作って Cookie 認証で操作を受け付ける場合は、**この自動防御の外**です。

自作 API のときは、「**この変更系 API、他サイトから叩かれたらどうなる？**」が問うべき質問。変更系を GET で作らない（GET は SameSite=Lax でも Cookie が付く側です）という基本もここで効いてきます。
...
- Server Actions は Origin 検証内蔵で、自作 API と GET の変更系が要注意ゾーン
