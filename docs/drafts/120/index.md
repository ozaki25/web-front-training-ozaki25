...
> **例示ベース**のテスト = 「この入力ならこの出力」という具体例を 1 つずつ並べる書き方

同値分割（同じ扱いの入力を仲間に分けて代表を試す）や境界値（境目の値を狙う）といった技法も、突き詰めれば「どの具体例を選ぶか」を上手にやる方法です。

例示ベースには構造上の限界があります。

- **書いた例の範囲しか確かめられない**
- どの例を書くかを決めるのは人間なので、人間が思いつかなかった入力は一度も試されないまま残る

実は上の `sortNumbers` にはバグがあります。それでもこの 2 本はどちらも成功します。このあとプロパティベースで捕まえます。

## 性質を書くという別の系統

具体例をひとつずつ増やす代わりに、**どんな入力でも成り立つはずの規則**をひとつ書く方法があります。

> **プロパティ（性質）** = どんな入力でも成り立つはずの規則。これを書くテストが**プロパティベーステスト**
...
プロパティベーステストのツールは、ランダムな入力を大量に自動生成して、性質が破れる入力を探します。

> **反例** = 性質が破れる入力

- 元祖は Haskell の **QuickCheck** というライブラリで、いまでは多くの言語に移植されている
- JS/TS では **fast-check** が定番
...
- **原因**: `sortNumbers` の中の `sort()` は、比較関数を渡さないと**要素を文字列に変換して辞書順で比べる**
- `[3, 1, 2]` のような 1 桁の例では正しく見える
- `[2, 10]` を入れると「"10" は "2" より小さい」と判定されて `[10, 2]` が返る
- 桁数の違う数を混ぜた例を人間が思いつかなくても、ランダム生成は当たり前のように混ぜてくる
...
ランダム生成で最初に見つかる反例は、大きな数が何十個も並んだ読みにくい配列だったりします。ツールはそこで止まりません。

> **shrinking**（反例の最小化）= 失敗が再現する範囲で反例を削っていき、できるだけ小さな形にしてから報告する処理

さきほどのバグなら、桁数の違う数が 1 つずつ入った短い配列のあたりまで削られます。
...
- 最終的な報告は「ランダムで何か壊れました」ではなく、「**この小さな具体例で失敗します**」になる
- 人間が手で書いた失敗例と同じ感覚でデバッグに入れる
- 見つかった反例を例示ベースのテストとして書き足しておけば、同じバグの再発も防げる
...
- **向いている**: 変換・エンコード・パース・ソート・日付計算のような、**入出力に規則がある純粋な関数**。フロントエンドでも、フォーム入力の正規化やクエリ文字列の組み立てなど、この形のロジックは意外とある
- **向いていない**: UI コンポーネントそのもの。「どんな入力でも成り立つ規則」を言葉にしにくい

難しさは、**良いプロパティを見つけること自体が設計**だという点です。

- **ありがちな失敗**: 期待値を出すために、テストの中で対象と同じロジックをもう一度書いてしまう。同じ勘違いを両側に写すだけで、何も確かめられない
- **コツ**: 出力そのものではなく「出力が満たす規則」を探す
