...
Web アプリでは、サーバーからデータを取ってくる処理が頻繁に発生し、この通信には時間がかかります。仮に、JavaScript が通信の完了を**その場で待つ**方式だったとします。
...
問題は、この「終わったら教えてもらう」を**コードでどう書くか**です。その書き方の中心にあるのが Promise です。
...
1 つなら十分に機能します。しかし「ユーザー情報を取得して、そのユーザーの注文を取得して、その注文の商品を取得する」のように**連鎖**すると、形が崩れます。
...
コールバックの中にコールバックが入り、右へ右へ深くなっています。これが**コールバック地獄**（callback hell）と呼ばれる形です。

処理の中身は「ユーザー → 注文 → 商品」という直線なのに、コードはそう読めません。さらに各段階のエラー処理を足すと、ネストはもっと膨らみます。

この問題を解決するために生まれたのが Promise です。

## Promise — 結果の「引換券」

**Promise**（プロミス）は、「まだ結果が返ってきていない非同期処理」を 1 つのオブジェクトとして扱う仕組みです。名前の通り「今は結果がないけれど、いずれ結果を届けると約束する」という**引換券のようなオブジェクト**です。
...
fulfilled と rejected をまとめて **settled**（決着済み）と呼びます。
...
`.then()` の中で値を返すと、その値を包んだ**新しい Promise** が返るので、さらに `.then()` を繋げられます。これが **Promise チェーン**です。

`.then()` が 2 つ続いているのは、`fetch` の結果が 2 段階（応答の受信 → 本文の変換）に分かれているためです。まずは「Promise を返す関数は `.then()` で連結できる」という形が読めれば十分です。
...
普段は `fetch` などの「Promise を返してくれる関数」を使うだけですが、作る側のコードを一度見ておくと、いつ rejected になるのかが分かります。
...
`resolve` を呼ぶか `reject` を呼ぶかは、**Promise を作る側が決めています**。何を失敗と見なすかは関数ごとに違い、たとえば `fetch` は 404 が返ってきても reject せず、通信自体が失敗したときだけ reject します。

だから、使う関数が「いつ reject するのか」を確認する習慣が、エラー処理の漏れを防ぎます。

## この先 — async/await

Promise チェーンでコールバック地獄は解消されましたが、`.then()` が連なるコードにはまだ独特の読み癖が要ります。そこで Promise の上に、同期処理と同じ感覚で上から下に書ける **async/await** という構文が用意されています。

現代のコードの大半は async/await の形で書かれていますが、**内部で動いているのは常に今日の Promise** です。チェーンの仕組みが分かっていれば、async/await も素直に読めます。

## まとめ

- 非同期処理は待たずに進み、終わったら教えてもらう仕組みで、書き方の中心が Promise
- Promise は結果の引換券で、pending → fulfilled / rejected の 3 状態をとる
- `.then()` で繋いで `.catch()` で一括エラー処理すると、深いネストが一直線になる
- いつ reject するかは作る側が決めるので、使う関数の仕様を確認する
