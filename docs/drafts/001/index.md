...
**2016 年の left-pad 事件**では、たった 11 行のパッケージが npm から削除されただけで、世界中のプロジェクトのビルドが壊れました。React や Babel のような巨大プロジェクトも影響を受けました。依存の連鎖のどこか 1 つが消えるだけで、全体が止まるのです。

セキュリティの問題もあります。パッケージの作者がアカウントを乗っ取られたり、悪意あるコードを紛れ込ませたりする事件が実際に起きています。あなたが `npm install` するとき、数百のパッケージの作者全員を信頼していることになります。これを<strong>サプライチェーン</strong>（供給網）のリスクと呼びます。

| リスク | 何が起きるか |
|-------|-----------|
| パッケージの削除・消失 | 依存先がなくなりビルドできなくなる |
| 悪意あるコードの混入 | 気づかないまま危険なコードを取り込む |
| メンテナンス放棄 | バグや脆弱性が放置される |
