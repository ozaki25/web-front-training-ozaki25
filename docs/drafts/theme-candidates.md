...
| アクセシビリティ | react-screen-reader-sample、アクセシビリティの祭典 2024・A11y Tokyo Meetup 参加 |
| テスト自動化・品質 | e2e-test-handson、Appium/Cypress 記事、JaSST'24 参加 |
...
| TypeScript | TSKaigi 2024-2026 連続参加、TS Developers Meetup |
| AI コーディング | MCP 活用事例 LT・「AIが変えるソフトウェア開発」参加、本教材のコンセプト自体 |
| デザイン・UX | Designship・Engineer Meets Design・RESEARCH Conference 参加、color-coordination-training、UX 教材 |

### インプットとアウトプットの非対称（はてなブログ・connpass 調査より）

参加イベントはフロントエンドに加えてスクラム（Scrum Fest Niigata 2026）、SRE（SRE Kaigi 2025/2026）、EM（EMConf JP）、Developers Summit と幅広い。一方、**自分の発信・教材として書くのは一貫して「仕組みを噛み砕く」フロントエンド系のみ**。プロセス系は関心はあれど教材化していない領域であり、この教材でも同じ線引きを維持するのが自然（Tier C 降格の根拠）。
