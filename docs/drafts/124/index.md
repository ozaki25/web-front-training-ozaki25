...
さっきまで見えていたページです。URL も同じです。それなのにリロードしただけで消えます。SPA をデプロイした人が最初にぶつかる定番のトラブルで、原因はバグではなく、**その URL に応えている相手がリロードの前後で入れ替わっている**ことにあります。
...
つまり `/products/42` という URL は、JavaScript がアドレスバーに書き込んだ結果です。サーバー上に `/products/42` に対応するファイルがあるわけではありません。**この URL はブラウザの中にしか存在しない**のです。
...
見た目は 404 なのにステータスは 200。この状態を **soft 404** と呼びます。人間の目には正しく見えても、ステータスコードを見て動く機械には別物として映ります。
...
リロード 404 を直したら今度は soft 404 が生まれる。どちらも「URL を解釈しているのが誰か」が変わったことの表と裏です。
...
存在しない URL の扱いも本物にできます。App Router では、存在しない商品 ID にアクセスされたときに `notFound()` という関数を呼ぶと、「見つかりません」の画面とあわせて **404 ステータスそのもの**を返せます。検索エンジンにも監視にも「このページは無い」と正しく伝わります。
