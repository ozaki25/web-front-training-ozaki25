...
- **海外製のツールやライブラリで繰り返し話題になってきた**: 日本語ユーザーだけが踏む不具合になりやすい
...
もう 1 つ、既存のコードでよく見かける書き方があります。React は composition イベントも `onCompositionStart` / `onCompositionEnd` として扱えるので、変換中かどうかを自前の state で持つ形です。

さきほどのコンポーネントに当てはめると、次の部分が変わります。
...
実装を頼むときは、「Enter で送信、Shift+Enter で改行、ただし日本語 IME の変換確定の Enter では送信しない」と最初から条件に入れられます。できたコードに `isComposing` も composition も見当たらなければ、日本語入力で誤送信しないかを実際に試してください。
