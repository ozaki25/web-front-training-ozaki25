...
この漏れ方は、外部から指摘されて直された報告がいくつもあります。ある政府系のサイトでは、ページのソースに埋め込まれたデータの中に、画面には出していない情報が残ったままになっていました。

見つけた人がやったのは、ソースを開いて中身を眺めることだけです。

セキュリティ診断の手順としても定番になっていて、Next.js で作られたサイトを調べるときは、まず埋め込みデータを開いて余分なフィールドがないか見る、と書かれた手引きがいくつもあります。

つまり、攻撃の腕前は要りません。**ソースを開けば誰でも読める**ので、外に出てしまうかどうかは、誰かが見たかどうかだけで決まります。
