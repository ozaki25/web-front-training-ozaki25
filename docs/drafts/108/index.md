...
- **従来モデル**: もともとキャッシュされる方向でした。Next.js 14 では取得結果が自動でキャッシュされ、「更新したのに画面が変わらない」というトラブルがよく起きました（15 で既定はキャッシュなしに変わりました）
- **新モデル**: 何も書かなければキャッシュされません。`"use cache"` と書いたものだけがキャッシュされます
...
- 従来モデルは既定でキャッシュ寄り・`fetch` ごとに指定、新モデルは書いたものだけ・`"use cache"` に統一
- どちらで動くかは `cacheComponents` の 1 行で決まる
