...
`"use cache"` を関数やコンポーネントに付けるのも、この「部分ごとに扱う」流れの一部です。キャッシュも、静的か動的かも、単位がコンポーネントに下りています。

その代わり、新モデルでは**非同期でデータを取るコンポーネントは `<Suspense>` で囲む**ことが必須になりました（囲み忘れるとビルド時にエラーになります）。「ここは後から動的に届く」という境目を、はっきり示させるためのルールです。

これは `"use cache"` を使うかどうかに関係ありません。`cacheComponents: true` にした時点で、非同期データを取るコンポーネントは `<Suspense>` か `"use cache"` のどちらかで囲む必要があります。

キャッシュを全く使わないプロジェクトでも、新モデルをオンにするなら `<Suspense>` は避けて通れません。
