...
- エラー推測・探索的テスト・チェックリストという 3 つの技法を知る
- 経験ベースは機械的に導けず、人が担う価値が高いと知る

## 仕様どおりのテストだけでは届かない穴

テストの多くは、仕様から機械的に作れます。入力の場合分け、条件の組み合わせ、画面の操作。どれも「仕様にこう書いてある」から導けるものです。
...
「毎回確認すべき項目」をリスト化し、1 つずつ試すのがチェックリストベースドテストです。過去の不具合や「ユーザーにとって何が重要か」から作ります。

- 例（フォーム）: 空で送信、最大文字数、二重送信、戻るボタン、全角入力…
- 探索的テストが自由なら、チェックリストは**毎回同じ観点を漏らさない**

エラー推測を「使い回せる形」にしたもの、と考えると近いです。

## 人の経験がものを言う

経験ベースは、テストする人の知識・好奇心・勘に大きく依存します。仕様という正解が外にある仕様ベースと違い、「何が起きうるか」を人が想像する必要があります。仕様や構造から機械的には導けないぶん、人が担う価値がいちばん高い領域です。

AI は、この想像の相棒として使えます。

- **壁打ち**: 「このログインフォームでありがちな失敗を挙げて」と指示すれば、エラー推測のリスト作りを手伝ってくれる
- **確認**: できたテストに、空・境界・異常系といったありがちな失敗が入っているかを、経験ベースの目で確かめる

最後に穴を見つけるのは人の経験です。候補出しは AI に任せ、判断は自分が持つ。この役割分担が、経験ベースでは特にはっきりします。
...
- 経験ベースは機械的に導けないので、候補出しは AI に任せ判断は人が持つ
