...
Web で「動き」をつけたいとき、選択肢はいくつかあります。選ぶ基準は「何がトリガーで動くか」です。

| トリガー | 方法 | 例 |
|---------|------|---|
| ホバーやクリック | CSS `transition` | ボタンの色が変わる、メニューが開く |
| 自動で動く・ループ | CSS `animation` | スピナーが回る、要素がふわっと現れる |
| スクロール | CSS Scroll-driven animations | プログレスバーが伸びる |
| JS から細かく制御 | Web Animations API / ライブラリ | 動きを途中で止める、速度を変える |
| 大量の描画 | Canvas / WebGL | ゲーム、粒子エフェクト |
