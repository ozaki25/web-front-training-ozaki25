...
- **Claude Code**（このセッションで動いている AI）: ファイルの読み書き、Git 操作、GitHub 操作を MCP サーバー経由で行っている
...
- AI は考える力はあるが手足が無かった。Tool Use で道具を渡せるようになった
- MCP は道具の差し込み口の標準規格。1 つ作れば対応ツール全部で使える
- 道具の名前と説明が AI の判断の質を左右する。説明文は指示の一部
- 道具を渡す = 権限を渡す。範囲を絞り、危険な操作は確認を挟む
