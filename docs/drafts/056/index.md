...
`@keyframes` は「開始と終了でどの状態にいるか」を書いた台本です。`from`/`to` は `0%`/`100%` と同じ意味で、2 段階なら `from`/`to`、3 段階以上なら `0%`/`50%`/`100%` のようにパーセントで書きます。`animation` はその台本を「何秒で、何回再生するか」という再生指示です。
