...
**AI チャットのパラパラ表示は、ほぼこの仕組み**です。生成 AI の API（ChatGPT や Claude）は、答えを生成しながら SSE 形式で少しずつ流しています。画面の文字がパラパラ出るのは、この `data:` の粒が届くたびに描画しているからです。
...
- HTTP は一往復で終わり、サーバーから話しかける手段は本来無い
- SSE は閉じないレスポンスで一方通行に流す仕組みで、AI チャットのパラパラ表示がこれ
- WebSocket は双方向の繋ぎっぱなし回線で、チャットや共同編集向け
- 選び方は「方向と頻度」で、一方通行に WebSocket は過剰
