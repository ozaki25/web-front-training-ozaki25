...
React は状態を変えるだけで画面が更新されます。毎回画面を作り直しているなら、遅いのではないか。

その鍵が <strong>DOM</strong>（ブラウザが HTML から作る画面の構造データ）です。DOM の操作は重く、書き換えるたびにレイアウトや描画のやり直しが走ります。React はこの重い DOM をできるだけ触らないために、<strong>仮想 DOM</strong>を使います。
