...
## 状態が変わるたびに画面を作り直して、遅くないのか

React では、状態（画面が覚えている値）を変えると画面が更新されます。書く側は「この状態のとき画面はこう」と宣言するだけで、更新の手順は書きません。ここで素朴な疑問が生まれます。毎回画面を「こうあるべき」と宣言し直すなら、全体を作り直して遅いのではないか、ということです。

その鍵が <strong>DOM</strong>（Document Object Model）です。DOM はブラウザが HTML から作る「画面の構造を表すデータ」で、`<button>` や `<p>` が親子関係を持つツリーになっています。`document.querySelector(...)` で操作するのがこれです。そして DOM の操作は<strong>重い</strong>。書き換えるたびにブラウザがレイアウトや描画をやり直すため、何十か所も直接書き換えると動作が重くなります。

React はこの重い DOM をできるだけ触らないために、<strong>仮想 DOM</strong>という仕組みを使います。

## 仮想 DOM — 軽いコピーで差分を見つける

仮想 DOM は、画面の構造を JavaScript のオブジェクトとして持ったものです。ただの軽いデータなので、作ったり比べたりするのは高速です。

状態が変わると、React は「新しい仮想 DOM を作る → 前の仮想 DOM と比べる → 変わった部分だけ本物の DOM に反映する」と動きます。次の図は、ボタンで要素が 1 つ増えたときの様子です。
...
リストの差分計算で、React は「前のどの項目が、新しいどれと同じか」を判断する必要があります。`key` はその項目を識別する目印です。固有の ID を `key` にすれば「これは前と同じ、これが新しく増えた」と正確に対応づけられ、増えた 1 件だけを DOM に追加できます。`key` がないと位置（上から何番目か）でしか対応づけられず、先頭に 1 件挿入しただけで全項目がズレたと判断され、全行が書き換わります。
...
- リストの <strong>`key`</strong> は差分計算の目印。固有の ID を使う
