...
変換器は「大文字始まりなら変数（関数）、小文字始まりなら文字列」という機械的なルールで振り分けています。小文字で `<orderCard>` と書くと `"orderCard"` という文字列になり、そんな名前の HTML タグは無いので画面に何も出ない。このルールは流儀ではなく、変換器が文字列と関数を振り分ける仕組みに由来しています。
