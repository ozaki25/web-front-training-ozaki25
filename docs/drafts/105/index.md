...
AI に「一覧画面のテストを書いて」と頼むと、たいてい「データが表示される」という成功のケースだけを試します。でも実際の画面には、成功のほかに読み込み中・0 件（空）・エラーといった状態があり、それぞれ見た目が違います。成功のケースだけテストして安心していると、エラー時に画面が真っ白、0 件のときに「該当なし」が出ない、といったバグは平気で残ります。
...
試すのは、状態そのもの（各状態で見た目が正しいか）と、遷移（次の状態へ正しく動くか）です。図の箱の 1 つ 1 つ、矢印の 1 本 1 本が、そのままテスト項目になります。
