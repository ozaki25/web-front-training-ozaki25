...
入力中の値が state にあるので、文字数カウントのように「**入力しながら値を使う**」ことが自由にできます。
...
入力中に再レンダリングが起きないぶん軽く、書き方も素の HTML に近いのが特徴です。ファイル選択（`<input type="file">`）は、ファイルの中身を React の state に入れられないため、**常に非制御**になります。
...
| 入力中の値を使えるか | 使える | 使えない（読みに行くまで不明） |
| 再レンダリング | 1 文字ごとに起きる | 起きない |
| 向く場面 | 入力中に値を使う<br>（文字数・即時検証・自動整形・ボタン制御・他欄と連動） | 送るだけ・シンプルなフォーム・ファイル入力 |

判断はこの一言に集約できます。

> **入力している最中の値が要るか？ それとも、送るときだけ読めれば十分か？**

- 検索ボックスで打つたびに候補を出す → 入力中の値が要る → **制御**
- 名前と住所を入力して送るだけ → 送信時に読めれば十分 → **非制御**でも足りる
...
> React Hook Form は基本的に**非制御（ref で値を集める）**で動き、入力中の再レンダリングを抑えて軽くしています。そのうえで、検証やエラー表示といった「制御の便利さ」を別の仕組みで足しています。
...
なお Next.js（App Router）では、サーバー側で送信を受け取る **Server Actions** という仕組みもあります。これは素の HTML フォームのように送信時に値をまとめて受け取る形で、考え方としては非制御に近いものです。

## まとめ

- 入力値の真実は 1 か所に決める（2 か所だと食い違う）
- 制御 = React の state が持つ（`value` + `onChange`）。入力中の値を使える
- `value` だけで `onChange` が無いと入力が固まる
- 非制御 = ブラウザが持つ（`defaultValue` + `ref`）。軽い、送信時に読む
- ファイル入力は常に非制御
- どちらが正解でもなく「入力中に値が要るか」で使い分け
