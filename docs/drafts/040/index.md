...
実は後者では、**ブラウザはページを移動していません**。移動したように見せているだけです。

## 本来のページ遷移 — 全部を捨てて全部を取り直す
...
- フルリロードは「全部捨てて全部取り直す」引っ越しで、白い画面はその作業時間
- SPA 遷移はクリックの乗っ取り・差分の取得と書き換え・History API の履歴操作でできている
