# SPA のページ遷移の正体 — リンクを押してもページは移動していない
...
どちらも「リンクを押して URL が変わって新しい画面が出る」のは同じです。何が違うのでしょうか。

実は後者では、**ブラウザはページを移動していません**。移動したように見せているだけです。この種明かしが今日のテーマです。
...
SPA（Single Page Application、1 枚のページでできたアプリ）は、この引っ越しをやめました。仕組みは 3 つの技の組み合わせです。
...
Next.js の `<Link>` は、この 3 つの技に加えて気の利いた仕事をしています。
