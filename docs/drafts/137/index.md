...
この増え方の違いは、会話やドキュメントでは O(n)（オー・エヌ）、O(1)（オー・イチ）と書かれます。線形探索は件数 n に比例して遅くなるので O(n)、ハッシュテーブルの探索は平均して件数によらず一定なので O(1)。記法の細かい定義まで覚える必要はなく、「O(1) は件数が増えても一定」とだけ覚えておくと、性能の会話についていけます。
...
変わったのは、`new Set(favoriteIds)` で Set を作る行と、`includes` を `has` に替えた行だけです。変換では全件を 1 回ずつしまうので配列の長さぶんの時間がかかりますが、それは一度きりで、ループの中の 5000 回の探索がすべて速くなります。
