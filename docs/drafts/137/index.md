...
つまり件数が 10 倍になれば、最悪の比較回数も 10 倍です。さらに先ほどのコードはこの探索を**ループの中で毎回**呼んでいるので、回数は掛け算で膨らみます。
...
厳密には、違う値がたまたま同じ番号になる**衝突**も起きます。ただ衝突しても同じ置き場に入った数個を比べ直すだけで済み、エンジンが置き場の数を増やして散らし直すので、「平均すれば一定」と言えます。
...
なお `Map` も同じハッシュテーブルの仕組みで動いていて、「キーから値を取り出す」が件数によらずほぼ一定の速さです。「含まれるか」だけでなく、ID をキーにして値もセットで引きたい場面では Map が選択肢になります。
...
`filter` の中に `includes` がある形を見つけたら、データが増える見込みを確認してみてください。直すと決めたら、AI にも「ループ内の includes を Set の has に置き換えて」と具体的に指示できます。
