...
コードは 1 行も間違っていないのに遅い。原因は `includes` の「探し方」にあります。

## Array.includes は先頭から順に比べる

配列は、値の入った箱が順番に並んだ構造です。`includes` で「含まれるか」を確かめるとき、できることは**先頭から 1 個ずつ「これか、違う。これか、違う」と比べていくこと**だけです。この探し方を**線形探索**と呼びます。
...
この増え方の違いは、会話やドキュメントでは O(n)（オー・エヌ）、O(1)（オー・イチ）と書かれます。線形探索は件数 n に比例して遅くなるので O(n)、ハッシュテーブルの探索は平均して件数によらず一定なので O(1)。この記法自体の詳しい話は別の機会に譲りますが、「O(1) は件数が増えても一定」とだけ覚えておくと、性能の会話についていけます。
...
変わったのは、`new Set(favoriteIds)` で Set を作る行と、`includes` を `has` に替えた行だけです。変換では全件を 1 回ずつしまうので配列の長さぶんの時間がかかりますが、それは一度きり。ループの中の 5000 回の探索がすべて速くなります。
