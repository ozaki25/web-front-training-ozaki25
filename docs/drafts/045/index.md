...
3 つの錐体の信号がそのまま「色」になるわけではありません。脳は錐体の信号を**足し引き**して、2 つのチャンネルで色を判別しています。
...
L と M の**差**が大きければ「赤っぽい」か「緑っぽい」が分かり、差がゼロに近ければ赤でも緑でもない（黄や白に近い）。S と L+M の**差**が大きければ「青っぽい」か「黄っぽい」が分かります。

この 2 チャンネルに加えて、L + M + S の**合計**で明るさを感じ取っています。つまり色覚は「赤-緑」「青-黄」「明暗」の 3 つの情報で成り立っています。
