...
なお開発中（Strict Mode）の React は、クリーンアップの書き忘れを早期に見つけるため、**初回の副作用をわざと「実行 → 片付け → もう一度実行」と 2 回動かします**。「console.log が 2 回出る」のは多くの場合バグではなく、この開発用の動作です。
...
- 依存配列が再実行の条件。中で使う値は依存に揃える
- 仕掛ける副作用には対になるクリーンアップを返す。開発中の 2 回実行は確認用の仕様
- 計算で済むものに useEffect は不要。「外の世界」が無ければ疑う
