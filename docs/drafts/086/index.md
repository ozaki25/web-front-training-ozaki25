...
この 2 つの単語の土台は **Promise** です。Promise は「まだ結果が出ていない非同期処理」を表す引換券のようなオブジェクトで、`fetch` のような時間のかかる関数が返してきます。

Promise の結果は本来 `.then()` で受け取りますが、チェーンが連なるコードには独特の読み癖が要ります。

**async/await** は、その Promise を**同期処理と同じ「上から下へ」の見た目で書ける構文**です。仕組みは Promise のままで、変わるのは読み書きのしやすさだけです。
...
大事なのは、`await` の行で**画面はフリーズしない**ことです。`await` に来ると関数の実行は**いったん中断**し、JavaScript はその間に他の仕事（クリックへの反応など）を進めます。

結果が出ると、中断した場所から続きが再開されます。
...
この中断と再開を `await` が肩代わりしてくれるので、待っていても画面は固まりません。
...
`Promise.all` の結果をさらに `await` で受ければ、async/await の書き方のまま並列化できます。

なお、1 つでも失敗すると `Promise.all` 全体が失敗扱いになります。「どれか失敗しても他の結果は欲しい」場合は `Promise.allSettled` を使い、各要素の成否を個別に見ます。

コードで `await` が縦に並んでいたら、「**順番に実行する必要があるか、同時にやれるか**」を確認してみてください。後の取得が前の結果を使うなら直列が正しく、無関係なら並列にできます。
...
`saveUser` が失敗しても、エラーはどの `catch` にも捕まらず、**unhandled rejection**（未処理の失敗）と呼ばれる状態になります。コンソールにエラーは出ますが、呼び出した側の処理は素通りします。

**保存できていないのに、ユーザーには成功したように見える**という、発見の遅れる事故につながります。

Promise を返す関数を呼ぶときは、原則 `await` するか `.catch()` を付けます。AI のコードを確認するときも、「Promise を返す関数の呼び出しに `await` が無い」は見つけやすいチェックポイントです。
...
`async` 関数の戻り値は Promise なので、`.then()` で受け取ることもでき、両者は自由に混ざります。

現在は**基本を async/await で書き、並列化したいところで `Promise.all` を組み合わせる**のが主流で、`.then()` チェーンを新たに書く場面は減っています。
...
- async/await は Promise の構文糖衣で、見た目は同期処理でも中身は Promise のまま
