...
ボタンそのものではなく、広い範囲のどこを押しても反応します。これを支えているのが、イベントの**伝わり方**の仕組みです。
...
通り道にある要素は、**自分に付けられたイベントハンドラでそのイベントを拾えます**。`<ul>` にハンドラを 1 つ付けておけば、中のどのボタンのクリックも拾えます。

モーダルの外側や一覧の行が「どこを押しても」反応するのは、こうして広い親要素でクリックを拾っているからです。
...
バブリングの実用例が**イベント委譲**です。1000 行のリストの各行にハンドラを 1000 個付ける代わりに、親に 1 つ付けて `e.target` で「どの行か」を判定します。

ハンドラの数が減るうえ、あとから行が追加されても付け直しが要りません。

### バブリングのデモ

下のボタンを押すと、イベントが親へ昇っていく様子がログに出ます。チェックを入れると、中間の div が `stopPropagation()` で伝わりを止めます。
...
<div class="c30-demo">
  <div class="c30-outer" onclick="document.getElementById('c30-log').textContent += '→ 外側の div のハンドラが拾った\n'">
    ...
    <div class="c30-inner" onclick="
      if (document.getElementById('c30-stop').checked) {
        ...
        document.getElementById('c30-log').textContent += '→ 中間の div が拾った（stopPropagation でここで停止）\n';
      } else {
  ...
  <pre class="c30-log" id="c30-log" aria-live="polite">（ボタンを押すと、イベントの伝わり方がここに表示されます）</pre>
</div>

ボタンの**外側**（中間や外側の div の余白）をクリックすると、バブリングがその要素から始まることも確かめられます。
...
| `e.stopPropagation()` | イベントの**伝播**（バブリング） | 親のハンドラに伝えない |
...
`stopPropagation` の代表例はモーダルです。「外側クリックで閉じる」を背景に付けると、**モーダルの中身のクリックもバブリングで背景に届いて閉じてしまう**ので、中身で伝播を止めます。
...
この 2 つを取り違えるのは定番の間違いです。「**既定の動作を止めるのか、親への伝播を止めるのか**」を言い分けられると、コードの意図も AI への指示も明確になります。

なお `stopPropagation` は、親側の「全体のクリックを監視したい」仕組み（分析計測やメニューを閉じる処理）まで黙らせてしまう副作用があります。多用は禁物で、まず設計で避けられないかを考えるのが定石です。

## React の onClick もこの仕組みの上にある

React の `onClick={...}` も、内部ではこのイベントの仕組みで動いています。
...
素のイベントの伝わり方を知っていれば、React で起きる「なぜ親まで反応するのか」「フォームがリロードされるのはなぜか」が、同じ図で説明できます。

## まとめ

- イベントは押された要素から親へ昇り（バブリング）、通り道の要素はどこでも拾える
- `e.target` は押された場所、`e.currentTarget` は拾った場所
- `preventDefault` は既定の動作を止め、`stopPropagation` は伝播を止める（別物）
- React の onClick はバブリングと委譲と合成イベントでできている
