...
JavaScript は、**コードを実行する流れ（スレッド）が 1 本しかない**言語です。一度に 1 つの処理しかできず、これを**シングルスレッド**と呼びます。

実行中の処理は**コールスタック**という場所に積まれ、関数を呼ぶと積まれて終わると降ります。スタックに何かが積まれている間、他の仕事は一切できません。

たとえばボタンのクリックで重い計算を回すと、その間は**ページ全体が固まります**。スクロールもクリックも効きません。
...
画面の描画もユーザーへの反応も、同じ 1 本のスレッドの仕事だからです。スタックが長時間占領されると、描画も入力も止まります。
...
JavaScript 自身は最後まで「一度に 1 つ」を守っています。並行で動いているように見えるのは、**待ちをブラウザに外注し、結果を行列で受け取っている**からです。

この行列を回し続ける係がイベントループです。

## setTimeout(fn, 0) の意味

この仕組みを踏まえると、次のコードの実行順を説明できます。
...
`0 ミリ秒`と指定しても、コールバックは**いったんタスクキューに並びます**。キューから取り出されるのは、いま実行中のコード（スタック）が全部終わってからなので、`"3"` が先です。

つまり `setTimeout(fn, 0)` は「0 秒後に実行」ではなく、「**いまの仕事が全部終わったら実行して**」という意味です。

## 行列は 2 種類ある — Promise は優先される

実は、実行待ちの行列は 1 つではありません。ここまで見てきたタスクキューは**マクロタスク**の行列で、それより優先される**マイクロタスク**の行列が別にあります。
...
非同期処理のコードで実行順が想定と違うと感じたら、「**同期 → Promise → setTimeout**」の 3 段を思い出してください。`async/await` もこの仕組みの上にあり、`await` の行から先はマイクロタスクとして後で実行される続きに変換されています。

### 触って確かめる

いま読んだクイズのコードを、このページ上でそのまま実行できます。書いた順は A → B → C → D です。
...
何度押しても A → D → C → B になります。これはたまたまではなく、イベントループの仕様どおりの順番です。

## この知識が効く場面

- **画面が固まる**: 原因は長時間スタックを占領する同期処理。「重い処理で UI がフリーズしている」と言葉にできれば、AI に分割や Web Worker（別スレッドで計算する仕組み）の検討を指示できる
- **実行順のバグ**: 「console.log の順番がおかしい」の多くは、マイクロ / マクロの優先度で説明がつく
- **非同期は並列ではない**: JavaScript は待ちを外注しているだけで、コードは常に一度に 1 つずつ実行されるので、複数の処理が同時に走って変数を壊す心配は基本的にない
