...
AI に React コンポーネントのテストを書かせると、決まってこんなコードになります。
...
AI が生成するテストはしばしば `data-testid` だらけです。動きはしますが、「**それ、getByRole で取れない？**」と問い直すと、テストが思想に沿った形に直り、ついでに HTML の a11y 不備（ラベル欠け、偽ボタン）が見つかることもよくあります。
