# Testing Library — ユーザーに見えるものでテストする
...
- `count` を `clickCount` にリネームしただけで、**動作は何も変わらないのにテストが失敗**する
- 逆に、表示部分を壊しても state さえ正しければ**成功したまま**になり得る
...
`data-testid` は HTML に**テスト専用の目印**を埋め込む方法です。確実に取れます。

ただしユーザーには見えも聞こえもしない属性なので、「ユーザーの使われ方に似せる」思想からは最も遠い。**役割でもラベルでもテキストでも取れないときの非常口**という位置づけです。
...
探し方だけでなく、操作も「ユーザーらしさ」を重視します。

- `userEvent.click`: クリック 1 発でなく、**ホバー → 押す → 離す**の列を再現
- `userEvent.type`: 1 文字ずつのキーイベントを発生
