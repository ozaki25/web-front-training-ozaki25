...
一番下ほど「何のために作るか」という話で、上にいくほど目に見える形に近づきます。

```mermaid
flowchart BT
  s1["戦略 Strategy — 何のためのアプリか"]
  s2["要件 Scope — どんな機能があるか"]
  s3["構造 Structure — どの画面からどの画面へ行けるか"]
  s4["骨格 Skeleton — 1画面のどこに何があるか"]
  s5["表層 Surface — どんな色や文字か"]
  s1 --> s2 --> s3 --> s4 --> s5
```
