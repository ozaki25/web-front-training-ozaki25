...
通信（1と2）は、この「ページを移る」という動作の一部として行われます。ブラウザはページを表示する道具なので、**通信は移動のついでに起きるもの**でした。
...
`fetch` がするのは、さっきの4つのうち1と2だけです。3の「今のページを閉じる」も、4の「新しいページを作る」も起きません。
...
図の並びはさっきと同じで、替わったのは真ん中にいるものだけです。

ページを移る動作に組み込まれているか、通信だけを単独で行うか。ここが分かれ目です。
