...
::: tip 専用の道具も増えた — useEffectEvent
React 19.2 からは、この問題のための公式 API **useEffectEvent** も使えます。「副作用の中の、最新の値で動いてほしい部分」を切り出すと、依存配列に入れなくても常に最新の state や props を読める関数になります。AI のコードに `useEffectEvent` が出てきたら、「stale closure 対策をしているな」と読めます。
:::
