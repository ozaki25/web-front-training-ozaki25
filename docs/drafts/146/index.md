...
通販サイトのカートアイコンには、商品を入れると件数のバッジが付きます。この「1 件以上のときだけバッジを表示する」を React で書くとき、定番の書き方に有名な罠があります。カートが空のときだけ、画面に謎の `0` が表示されるバグです。
...
`count` が `3` のときは期待どおり、バッジに `3` と表示されます。ところが `count` が `0` のとき、「何も表示されない」はずが、バッジの位置に **`0` という文字がそのまま表示されます**。
...
JavaScript では、`if (count)` のように真偽値でない値を条件の場所に置くと、その値を true 相当か false 相当かに変換して判定します。true 相当として扱われる値を truthy、false 相当として扱われる値を falsy と呼びます。
...
この 8 つ以外は、すべて truthy です。意外なのは、空配列 `[]` や空オブジェクト `{}` も truthy だという点です。「中身が空なら false 側だろう」という直感は通用しません。
...
ところが `{count && <span>...</span>}` の `count` が `0` のとき、式の結果は `false` ではなく数値の `0` です。数値は「何もない」扱いにならず、そのまま文字として描画されます。これが、0 件のときだけ画面に `0` が出るバグの全体像です。
...
どちらでも直りますが、覚えることは一言です。**`&&` の左側に、`0` になりうる数値をそのまま置かない。**
...
空配列 `[]` 自体は truthy ですが、ここで条件に置いているのは `items.length` です。空配列の `length` は `0` で、`0` は falsy なので、一覧の代わりに `0` が表示されます。直し方も同じで、`items.length > 0` と比較して真偽値にします。
...
「値が未設定のときだけデフォルト値を使いたい」場面では、`??` のほうが向くことがあります。たとえば「未取得なら『-』、0 件なら `0` と表示したい」とき、`count || "-"` では `0` まで「-」に化けますが、`count ?? "-"` なら `0` は `0` のまま表示されます。falsy 全部を「値なし」とみなすのが `||` と `&&`、`null` と `undefined` だけを「値なし」とみなすのが `??`、という区別です。

## AI のコードを確認する視点

条件表示のコードを見かけたときの確認ポイントは、「`&&` の左側は真偽値か、それとも `0` になりうる数値か」の一点です。AI に条件表示を任せたあとも、「`count` が 0 のときの表示を確認して」「`&&` の左側は比較演算子で真偽値にして」と指示できれば、このバグは入り口で防げます。

## まとめ

- falsy は `false`・`0`・`""`・`null`・`undefined`・`NaN` など 8 つだけで、空配列や空オブジェクトは truthy
