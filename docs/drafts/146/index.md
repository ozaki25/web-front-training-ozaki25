...
通販サイトのカートアイコンには、商品を入れると件数のバッジが付きます。この「1 件以上のときだけバッジを表示する」を React の定番の書き方で書くと、カートが空のときだけ画面に**謎の `0` が表示される**バグに出会うことがあります。
...
`{count && <span>...</span>}` は React の条件表示の定番の形で、AI まかせで作った画面のコードにもよく入っています。このバグは React 固有の話ではなく、JavaScript の値の性質と `&&` の動きを順に追うと原因がわかります。
...
AI に条件表示を書かせたときも、「`count` が 0 のときの表示を確認して」と一言添えれば、このバグは入り口で防げます。
...
- falsy は `false`・`0`・`-0`・`0n`・`""`・`null`・`undefined`・`NaN` の 8 つだけで、空配列や空オブジェクトは truthy
