...
「Google でログイン」の正体は、**OIDC による認証 + OAuth による最小限の認可**のセットだったわけです。
...
リダイレクトの検証、トークンの保管、失効、セッション管理。OAuth/OIDC の周辺は**間違えると即事故になる落とし穴が大量にある**領域で、世界中の専門家が叩いてきた実装に乗るのが正解です。
