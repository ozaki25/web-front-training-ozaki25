...
サーバーの関数が `return` した値が、そのままクライアントの `state` に現れる。サーバーとブラウザの往復が、関数の引数と戻り値という**普通のプログラミングの形**に収まっています。
...
Server Action に認証チェックも値の検証も無かったら、それは「鍵のかかっていない裏口」です。「このアクション、直接呼ばれても大丈夫？」が合言葉です。
