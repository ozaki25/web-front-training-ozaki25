...
- JSON だからこそ、実行時に速度や色を変えたり操作と連動できると知る
...
その多くは **Lottie** という形式で作られています。デザイナーが Adobe After Effects や Figma で作ったアニメーションを、そのまま Web やアプリに持ち込む仕組みです。

なぜこのアニメーション、動画や GIF ではなく、数十 KB の JSON ファイルで届くのでしょうか。
...
命令を記録するだけなので、典型的な Lottie アニメーションは数十 KB で収まります。同じ滑らかさの GIF や動画に比べて、大きさが数分の一からときに 6 分の 1 程度で済むと言われています。
...
`layers` の中の `ks` が、そのレイヤーの不透明度（`o`）や大きさ（`s`）を、時刻ごとの値の組として持っています。この時刻と値の組がキーフレームです。
...
作り方そのものを送っているので、動画や GIF ではできないことができます。

- **速度や再生位置を変えられる**: `setSpeed()` で倍速にしたり、`goToAndStop()` で途中の状態で止められる
- **色や向きを実行時に変えられる**: 図形の色も命令の一部なので、ダークモードに合わせて塗り色を差し替えられる
- **操作と連動できる**: スクロール量やボタンのクリックに合わせて、途中で再生・停止・巻き戻しができる

読み込み中にくるくる回るアイコンや、いいねボタンを押した瞬間だけ再生されるアニメーションは、この「途中で止めて、また動かせる」性質を使っています。動画ではフレームを送り続けるしかなく、ここまで細かい制御はできません。

## Web と Next.js での扱い方

現在の Web 向け実装は、Rust と WebAssembly で作られた `@lottiefiles/dotlottie-web` と、その React 向けラッパーである `@lottiefiles/dotlottie-react` が主流です。
...
`{ ssr: false }` を付けて動的 import にしているのは、思いつきの書き方ではありません。ここまで見てきたとおり、Lottie の再生は「JSON を読み込み、ブラウザの `svg` や `canvas` に図形を描く」処理です。

Server Components はサーバー側で HTML を組み立てるだけで、そこに `canvas` も `svg` を描く実行環境もありません。だから Lottie の再生コンポーネントは、必ずブラウザ側（Client Component）で、しかもサーバーでの事前実行をスキップして動かす必要があります。
...
ここで効くのが、さきほどの「命令だから実行時に変えられる」性質です。設定を JavaScript 側で読み取り、再生インスタンスを直接止めます。
...
- 命令だからこそ小さく、拡大してもぼやけず、速度や色を実行時に変えられる
