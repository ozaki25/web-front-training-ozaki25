...
検索ボックスに 1 文字打つたびに、下の候補が絞り込まれていく。EC サイトの商品検索や管理画面のユーザー検索で、よく見る動きです。

この検索ボックスに、素早く「re」→「rea」→「react」と打ち込んだとします。画面にはいったん「react」の検索結果が並んだのに、その直後、なぜか「re」の検索結果に置き換わってしまう。打ち直すと直ることもあれば、また起きることもある。

操作は何も間違っていません。それでも、処理のタイミング次第で結果が壊れる。この現象を**競合状態**（race condition）と呼びます。複数の処理が競走していて、どれが先に終わるかで結果が変わってしまう状態のことです。もともとは並行処理の用語で、フロントエンドでは「入力のたびに通信する」画面がこの状態になりやすい代表格です。
...
## 対策その 2. リクエストそのものを中断する

無視する方式では、古いリクエストの通信自体は最後まで続きます。使わないと決めたレスポンスを、サーバーは作り続け、回線を通って届けてくる。この無駄ごとやめさせるのが **AbortController** です。
