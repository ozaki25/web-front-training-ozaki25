...
メモ化は、一言で言うと「**前と同じ答えなら、計算し直さずに前の答えをそのまま使う**」ことです。

身近な例で言えば、レストランの注文表。毎回メニューを最初から全部読み直す人はいません。「さっきと同じで」と言えば済む。メモ化はこの「さっきと同じ」をコードで自動化する仕組みです。

React では、画面に変化があるたびにコンポーネント関数が丸ごと再実行されます。入力欄に 1 文字打っただけでも、その関数の中の**すべての計算が毎回走る**のが既定の動作です。メモ化は、その中で「前と同じ入力のまま変わっていない計算」を見つけて、**再計算をスキップ**させます。
