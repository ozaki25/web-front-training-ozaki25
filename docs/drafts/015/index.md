...
メモ化は、一言で言うと「**前と同じ答えなら、計算し直さずに前の答えをそのまま返す**」仕組みです。

たとえば、商品一覧を価格順に並べ替える処理を考えます。商品が 1 万件あると、ソートにはそれなりの時間がかかります。

**メモ化なし（毎回計算する）**:

1. ユーザーが「a」と打つ → 商品 1 万件をソートする（重い）
2. ユーザーが「ab」と打つ → **また**商品 1 万件をソートする（重い）
3. ユーザーが「abc」と打つ → **またまた**ソートする（重い）

商品データは変わっていないのに、入力するたびに毎回ソートし直しています。

**メモ化あり（前の答えを覚えている）**:

1. ユーザーが「a」と打つ → 商品 1 万件をソートする（1 回目は計算する）→ **結果を覚えておく**
2. ユーザーが「ab」と打つ → 商品データは前と同じ？ → 同じ → **覚えていた結果をそのまま返す**（速い）
3. ユーザーが「abc」と打つ → 同じ → **またそのまま返す**（速い）
...
仕組みの核は「**材料が変わったかどうかを比べる**」だけです。変わっていなければ前の答えを返す。変わっていたら計算し直して、今度はその答えを覚える。これを React の API でやるのが `useMemo` / `useCallback` / `React.memo` です。
