...
「a」と 1 文字打っただけなのに、なぜ画面全体が作り直されてしまうのか。原因は React の**再レンダリング**の仕組みにあり、それを抑える道具が**メモ化**（前と同じ材料なら計算をやり直さない仕組み）です。原因から順に見ていきます。
...
渡す先はただの `<button>` 要素で、`memo` されたコンポーネントではありません。つまりこの `useCallback` は**効いていません**。メモ化はそれっぽく書かれていても、正しく効いているとは限らない。ここまでの内容を踏まえると、効いていないケースを見抜けるようになります。
...
::: details useMemo の有無で入力の重さがどう変わるか、手元で試せます

画面下の REPL を開き、**TSX** タブに以下を貼って ▶ 実行してください。

**useMemo なし版** — 入力欄に文字を打つと、1文字ごとにもたつきます。
...
**useMemo あり版** — 貼り替えて再実行すると、同じ操作が一瞬で反映されます。
