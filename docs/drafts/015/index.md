...
たとえば、商品一覧を価格順に並べ替える処理を考えます。商品データが変わっていないのに、入力欄に 1 文字打つたびに毎回ソートし直すのは無駄です。メモ化をかけると、「**前回と同じ商品データが来たから、前回ソートした結果をそのまま返す**」と判断し、ソート処理をまるごとスキップします。

仕組みの核は「**入力が変わったかどうかを比べる**」だけです。変わっていなければ前の答えを返す。変わっていたら計算し直して、今度はその答えを覚える。これを React の API でやるのが `useMemo` / `useCallback` / `React.memo` です。
