...
どちらを選んでも副作用があります。FOIT は「読めない時間」を生み、FOUT は「切り替わる瞬間」を生む。冒頭の現象はバグではなく、ブラウザがこの二択のどちらかを実行した結果です。
...
| `block` | 3 秒ほど文字を隠して待ち、間に合わなければ代替フォントで表示する。FOIT 寄り | 代替フォントでは意味が壊れるアイコンフォントなど |
