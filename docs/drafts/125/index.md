...
共有キャッシュは、サーバーとブラウザの間に立つ中継サーバーが持つ保存場所です。

- 代表は **CDN**（世界各地に配置されたキャッシュ専用のサーバーから、ユーザーに近い場所でコンテンツを配る仕組み）
- 1 人へのレスポンスを保存しておき、同じ URL へ来た**別の人にも**配る
...
サーバーが人を見分けられるのは、ブラウザがリクエストに **Cookie** を付けて送っているからです。

- **Cookie**: サーバーが発行する会員証のような小さなデータ。以降のリクエストに毎回添えられる
- サーバーは Cookie を見て「これは A さんだ」と判断し、A さんの情報でページを組み立てて返す

つまり、ここにずれがあります。

- **出し分けの手がかり**: Cookie にある。**URL には誰のページかという情報が入っていない**
- **共有キャッシュの判断基準**: 「同じものだ」と判断する基準（キャッシュキー）は、基本的に URL

このずれが事故の入り口になります。

## 事故が起きる流れ

ずれが実際の事故になるのは、個人向けのレスポンスに「キャッシュしてよい」という指示が付いたときです。指示が付く経路は 2 つあります。

- サーバーがレスポンスヘッダーで許可してしまう
- CDN 側の設定で、特定のパスを強制的にキャッシュしている
...
やっかいなのは、B さんのリクエストがオリジンサーバーに届いていないことです。

- 認証のコードにバグはない
- サーバーのログにも異常が残りにくい
- だから、アプリのコードだけを調べていても原因にたどり着けない
...
マイページのように個人情報を含むレスポンスには、`private` か `no-store` を付けます。

- ネットカフェのような共有端末では、ブラウザに残ることも避けたい。その場合は、より厳しい `no-store` を選ぶ判断もある
- 逆に、個人情報を含むレスポンスに `public` や長めの `max-age`（使い回してよい秒数の指定）が付いていたら危険信号
...
では `Vary: Cookie` を付ければ人ごとに分けられるかというと、理屈の上ではそのとおりです。

- ただし Cookie の値はほぼ全員違うので、保存した結果が次の人に当たることがほぼない
- つまり、共有キャッシュに置く意味が消える

個人向けのレスポンスは、分けて置くのではなく、そもそも共有キャッシュに載せないのが実務の基本です。

## 共有キャッシュに載せるものの仕分け

事故の背景には「CDN でキャッシュすれば速くなる」という高速化の圧力があります。

- 圧力自体は正しい。全員に同じものを配るページで、CDN は絶大な効果を出す
- 問題は、速さを求めるあまり、個人向けのページまでまとめてキャッシュの対象にしてしまうこと

だから本当に考えるべきは、キャッシュ機能の使い方より先に、この仕分けです。
...
- CDN 側には「このパスは強制的にキャッシュする」といった設定がある
- そこでフレームワークの付けたヘッダーを上書きすれば、同じ事故が起きる
- **フレームワークが守ってくれる範囲と、インフラの設定で壊せる範囲は別物**

だからこそ、仕組みを知っていれば言葉で伝えられます。

> 「このページはログインユーザーの個人情報を表示するので、共有キャッシュに保存されないように Cache-Control は private か no-store にしてください」

AI に実装を頼むときも、CDN の設定をチームで相談するときも、この一言で要件が正しく伝わります。
