...
アプリを何もインストールしていないのに、こういう重たい処理がなぜブラウザだけで動くのか。その答えが今日の主役、**WebAssembly**（Wasm）です。
...
WebAssembly の実体は、0 と 1 が並んだ**バイナリ形式**のファイルで、拡張子は `.wasm` です。JavaScript と違って、ブラウザの開発者ツールでソースコードをそのまま読むことはできません。ただし動く場所は無制限ではなく、JavaScript と同じ**サンドボックス**（ブラウザが用意した隔離された実行環境）に閉じ込められています。パソコンのファイルやネットワークに、WebAssembly が勝手にアクセスすることはありません。
...
::: details WebAssembly をもっと厳密に言うと

WebAssembly の正体は、スタックベースの仮想マシン向けの命令をまとめたバイナリ形式です。2017 年に主要ブラウザが対応し、2019 年には W3C の正式な標準になりました。

CPU の実際の機械語そのものではなく、どの CPU でも同じように解釈できる共通の命令セットです。ブラウザは実行時に、手元の CPU 向けの命令へその場で変換します。Intel の CPU でも ARM の CPU でも、同じ 1 つの `.wasm` ファイルがそのまま動くのはこのためです。

変換元になる言語も C++ や Rust に限りません。WebAssembly を出力できる言語は、ほかにも増え続けています。
