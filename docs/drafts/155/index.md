...
Canvas はそうではありません。中身はただの色のついた点（ピクセル）の集まりで、四角形や丸を描いても、描いた瞬間に「絵」になるだけです。「ここに四角形があった」という情報はブラウザに残らないので、あとから 1 つだけ動かすことはできません。何かを動かしたいときは、画面をまるごと消してから、新しい位置でもう一度全部を描き直します。
...
クリックされたのがどの図形かも、ブラウザは教えてくれません。クリックされた座標が、自分が描いた図形の内側にあるかどうかを自分で計算する必要があります。Figma のようなアプリの内部実装が複雑になる理由のひとつは、こうした判定をすべて自前で持っていることです。

そして Canvas への命令（`fillRect` など）は、DOM の書き換えや fetch と同じく、ブラウザが JavaScript に開放している機能（**Web API**）の一種です。WebAssembly は Web API を直接呼べないので、実際の流れは「WebAssembly が次に描く内容を計算し、JavaScript が Canvas に命令してピクセルを塗る」という形になります。Canvas も、JavaScript と WebAssembly が役割を分けて共存する具体例のひとつです。
:::
