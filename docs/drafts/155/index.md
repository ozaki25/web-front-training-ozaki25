...
このコードが動いてしまうと、あなたが銀行にログイン中のブラウザでは、スクリプトが口座情報を読み取って外部へ送信できてしまいます。個人情報や認証情報の盗み取りに直結します。
...
答えは **「タグが HTML 標準の一部だから、ブラウザが許可している」** です。Web の初期段階から、画像の外部リンクや CDN の CSS 読み込みは当たり前でした。

このような歴史的背景があるため、これらのタグの読み込みは制限していません。
