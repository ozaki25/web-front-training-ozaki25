...
雲行きが変わるのは、そのあとです。チェックボックスで 1 件を完了にする、タイトルを 1 件だけ書き換える、クリックした 1 件の詳細を出す。機能が増えるたびに「**この ID のタスクはどれか**」を配列の中から探す処理が必要になります。
...
このコードは正しく動きます。ただ、よく見ると `toggleDone` も `renameTask` も、やっていることの半分は「ID が一致する 1 件を探す」です。ここに「クリックした 1 件の詳細を出す」機能を足せば、`tasks.find((task) => task.id === selectedId)` がもう 1 か所増えます。同じ探すロジックが関数の数だけ重複し、しかもどれも配列の先頭から 1 件ずつ照合します。タスクが数千件になれば、1 件を触るたびに数千回の比較が走ります。
...
オブジェクトは「キーと値の対応表」です。得意なのはキーを使った操作で、`tasksById[id]` と書けば、何件あっても探すループなしで目当ての 1 件に届きます。キーは重複できないので、同じ ID のデータが二重に紛れ込むこともありません。

代わりに、順番を使う操作は苦手です。「3 番目のタスク」という取り出し方はできませんし、並び順を情報として持たせる用途にも向きません。
...
変換は届いたときの 1 回だけです。以降の「この ID の 1 件を参照・更新したい」はすべて `tasksById[id]` で済み、関数ごとに重複していた探すロジックが消えます。詳細表示を足すときも `tasksById[selectedId]` と書くだけです。一覧表示が必要な場面だけ `Object.values` で配列に戻します。

## 順番も大事なときの持ち方

1 つ注意があります。JavaScript のオブジェクトでは、`"1"` や `"42"` のような整数に見える文字列のキーは、追加した順ではなく数の昇順で列挙される仕様です。それ以外の文字列キーは追加した順に並びます。API が数値の ID を返す場合、`Object.values` で戻した配列の順番は届いた順と一致しないことがあります。
...
「中身はキーで引ける対応表、順番は配列」という分担で、両方の得意分野を使えます。状態管理ライブラリのドキュメントでも紹介される定番の形です。

なお、対応表の入れ物にはオブジェクトのほかに `Map` もあります。Map はキーの追加と削除を大量に繰り返す用途に最適化されていて、文字列以外の値もキーにできます。React の状態として持つなら、スプレッド構文でそのまま更新できるオブジェクトのほうが手数が少なく、まずはオブジェクトで困りません。
...
配列のままが間違いというわけではありません。数十件の一覧を表示して、更新箇所も 1 つだけなら、配列のままで十分です。判断の分かれ目は「ID を指定した参照・更新が、コードのあちこちに出てくるか」です。

## この知識が効く場面

- **設計を頼むとき**: 「この一覧は ID 指定の更新が多いから、ID をキーにしたオブジェクトで持って。表示のときだけ配列に戻して」と最初から指定できる
- **コードを評価する目になる**: 状態を更新する関数のたびに `find` や `map` で ID を探す形が並んでいたら、処理の書き方ではなくデータの持ち方から見直す
- **会話についていける**: 「正規化して持つ」「ID で引ける形にする」という言い回しは、この持ち方を指していることが多い
