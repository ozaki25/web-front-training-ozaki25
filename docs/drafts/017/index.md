...
- `?.` と `??` がエラーで画面を落とさないための構文だと知る

## AI が書く典型的な 1 ファイル

AI に「商品カードのコンポーネントを作って」と頼むと、こんなファイルが返ってきます。
...
これらは React 特有の機能ではなく、すべて **JavaScript の標準構文**です。そして AI が書く React コードのほぼ全行に登場します。

読めない記号が残っていると、その行は「おまじない」になります。おまじないのままでは、AI に修正を指示することも、返ってきたコードの違和感に気づくこともできません。今日はこの 1 ファイルを全行読めるようにします。
...
```mermaid
flowchart LR
  A["Badge.tsx<br>export function Badge"] -- "公開されたものを" --> B["ProductCard.tsx<br>import { Badge }"]
...
なお、import の波括弧は次に説明する分割代入と見た目がそっくりですが、**別の仕組み**です。「named export を名前で選んでいる」専用の記法だと覚えてください。
...
`useState` は「現在の値」と「更新する関数」のペアを配列で返します。配列の分割代入は位置で取り出すので、1 番目を `count`、2 番目を `setCount` と、**呼ぶ側が自由に名前を付けられます**。`useState` がオブジェクトではなく配列を返すのは、この「名前を自由に決められる」性質を活かすためです。
...
ポイントは、どちらも**元のデータを書き換えず、新しいオブジェクトや配列を作っている**ことです。React は「前と同じオブジェクトかどうか」で変更を検知するため、書き換えるのではなく新しく作って渡すのが React の流儀です。AI が `user.name = "鈴木"` ではなく `{ ...user, name: "鈴木" }` と書いてくるのはこのためです。

JSX では、props をまとめて渡す使い方も頻出します。

```tsx
function IconButton(props: { label: string; onClick: () => void }) {
  // 受け取った props を <button> にそのまま配り直す
  return <button {...props} aria-label={props.label} />;
}
```

### レスト — 残りを集める

同じ `...` でも、分割代入の中で使うと意味が逆になります。取り出した**残り全部**を 1 つに集めるので、**レスト**（rest = 残り）と呼ばれます。
...
| `a \|\| b` | `a` が falsy（偽とみなされる値） | `b` になる | `b` になる |

「割引率 0%」や「空文字の入力値」は正当な値です。`||` だとこれらまで置き換えられてしまうため、「値が無いときだけ」の意図なら `??` が適切です。AI のコードで `||` が使われていたら、`0` や `""` が来る可能性を考えてみてください。
...
クラス名の組み立てなど、条件と文字列が混ざる場面で力を発揮します。
...
最初のファイルに戻ると、すべての行が今日の構文でできています。
...
- `` `¥${price.toLocaleString()}` `` — 文字列への埋め込み
