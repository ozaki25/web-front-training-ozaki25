...
## React のコードが読めない、本当の理由

React のコードを前にして「何が書いてあるか分からない」と感じるとき、つまずきの正体は React ではなく、**土台の JavaScript の構文**であることがほとんどです。

たとえば、ありふれた「商品カード」の一部を見てみます。**今は一行ずつ理解できなくて構いません**。眺めるだけで大丈夫です。
...
読めない記号が残っていると、その行は「おまじない」になります。おまじないのままでは、コードの違和感にも気づけません。今日はこの 1 ファイルを、全行読めるようにします。
