...
しかし「dev って何？」と聞かれると、意外と答えられません。`npm` も `run` も分かるのに、`dev` の正体は曖昧なままです。

実は `package.json` に答えがあります。
...
`dev` や `build` と違い、lint は**動作には影響しません**。指摘を無視してもアプリは動きます。しかし「動くけれど問題のあるコード」を機械が見つけてくれる価値は大きいものです。
...
scripts はプロジェクトの「**操作マニュアルの目次**」です。初めてのプロジェクトでも、`scripts` を読むだけで動かし方の全貌が掴めることもあります。
...
- dev で動いても build で落ちることがあるので、build を通して安心
- scripts を読むと、プロジェクトの動かし方と技術スタックが分かる
