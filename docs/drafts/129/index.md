...
タイピングのキー入力は 100〜300ms ほどの間隔で連続します。通信が返ってくるより速いペースで発生するイベントに、毎回そのまま反応しているのが問題です。この「速すぎる操作」への調整には 2 つの型があります。
...
検索機能の実装を AI に任せるとき、出てきたコードを確かめる観点はこうです。

- **入力のたびに fetch がそのまま飛んでいないか**: 頻度の調整が入っていなくても検索としては動いてしまうので、動作確認だけでは気づけない
- **useEffect の中に setTimeout と clearTimeout の対があるか**: これが debounce が入っているサイン
