...
- エラーは種類・説明・スタックトレースの 3 部品で、説明は定型文として訳せる
- 最頻出は「undefined の手前が無い」で、name ではなく xxx.name の xxx を疑う
- スタックトレースは上が現場・下が呼び出し元で、最初の自分のファイルを見る
- Next.js はブラウザとターミナルの両方を見て、読解 1 行を添えて AI に渡す
