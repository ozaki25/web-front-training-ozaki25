...
コードでは、`as` を使って API レスポンスに型を付けるパターンがよく出てきます。`const data = (await res.json()) as User` のような形です。これは「型が付いているように見えるが、実行時には何も検証していない」コードです。

「`as` ではなく Zod で検証して」と指示できるのは、型消去を知っている人の判断です。

## まとめ

- TypeScript の型はビルド時に消える（型消去）ので、実行時には保護がない
