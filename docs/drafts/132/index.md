...
コメント欄に入力された文字列がそのまま HTML に埋め込まれると、閲覧者のブラウザで攻撃者の JavaScript が実行されてしまいます。この攻撃が、クロスサイトスクリプティング、略して **XSS** です。
...
1 か所でも漏れれば XSS は成立します。そこで、注入を防ぐ対策とは別に、発想の違う防御をもう 1 つ重ねます。

**万一スクリプトが注入されても、ブラウザに実行させない**。それが CSP です。

> **多層防御**: 性質の違う守りを重ねる考え方（defense in depth）。エスケープが第一の防御で、CSP はそれが破られたときの第二の防御

## ブラウザに許可リストを宣言する CSP

> **CSP**: Content-Security-Policy という HTTP レスポンスヘッダー。サーバーがページを返すときに付けると、「このページで実行してよいスクリプトの出どころはここだけ」とブラウザに宣言できる
...
「自分のサイトのファイルとして配信されるスクリプトは許可し、HTML に直接書かれたスクリプトは信用しない」という線引きが、注入されたコードだけを狙い撃ちでブロックします。

## 正当なインラインスクリプトを許可する nonce と hash
