...
この違いが「grid ファースト」という最近の流れにつながっています。grid は親がレイアウトを宣言するので挙動が予測しやすく、1 方向の並びでも `grid-template-columns: auto 1fr auto` のように書けます。flex でないと困る場面——子の中身に応じてサイズを柔軟に変えたいとき——以外は grid を選ぶ、という考え方です。

## 現在地

横並びの歴史を振り返ると、float と inline-block はどちらも「横に並べるために作られたものではない」仕組みの転用でした。flex と grid は最初からレイアウトのために設計されているので、ハックがいりません。

| 時代 | 方法 | 特徴 |
|------|------|------|
| 2000 年代 | `float` | 回り込みの転用。clearfix ハックが必要 |
| 2000 年代後半 | `inline-block` | HTML の改行が隙間になる |
| 2012 年〜 | `flex` | 子が交渉してサイズを決める。1 方向が得意 |
| 2017 年〜 | `grid` | 親がトラックを定義する。格子も 1 方向もいける |

AI が生成した CSS に `display: flex` や `display: grid` が入っているのを見かけたら、「float や inline-block のハックなしに横並びができるようになったんだな」と思い出してください。そして flex と grid のどちらが使われているかを見て、「子に任せているのか、親が決めているのか」を読み取れると、レイアウトの意図がわかるようになります。
...
- **flex** は子がサイズを交渉する仕組みです。柔軟ですが、子の中身が変わると他の子にも影響します
- **grid** は親がトラックを定義する仕組みです。挙動が予測しやすく、格子も 1 方向もこなせます
- 迷ったら grid。子の中身に応じてサイズを柔軟に変えたいときだけ flex、と覚えておくとシンプルです
