...
React でユーザーの一覧を表示するコードは、AI に頼んでも自分で書いても、ほぼ決まってこの形になります。
...
::: tip `=> (` と `=> {` の違い
`map` の中の `=> (` の丸括弧は、複数行の式をまとめて返す書き方で、JSX が 1 行で収まらないときの定番の形です。`=> {` の波括弧にすると、`return` が必要になります。
:::

一覧を作るなら、繰り返しの構文 `for` のほうが素直に思えます。それでも React の一覧が決まって `map` になるのには、JavaScript の文法上の理由があります。
...
`map` が作った配列が画面になる仕組みはシンプルで、React は **JSX の配列を渡されると順番に描画する**というルールを持っています。
...
配列の例にだけ付いている `key="a"` は、このあと説明する key という属性です。
...
一覧表示は繰り返し処理ではなく、データの配列を JSX の配列へ**変換**する処理です。その変換のためのメソッドが `map` です。
...
一覧のデータは、追加・削除・並べ替えで変化します。そのとき React は「前回の一覧」と「今回の一覧」を見比べて、変わった部分だけを画面に反映します。

名札が無いと、並べ替えただけなのに「全部別人になった」と誤解されます。無駄な作り直しが起きたり、入力欄の中身が別の行に紛れ込んだりします。
...
id が無いデータに対しては、`key={index}`（配列の何番目か）で済ませる書き方もあります。並べ替えや途中への追加が無い固定の一覧なら動きますが、並びが変わる一覧では「何番目か」と「誰か」がズレて表示が崩れます。

「この一覧、並び変わるけど key は大丈夫？」と確認できると、AI のコードを見る目が一段上がります。
...
絞り込んでから JSX に変換する、という 2 段構えです。データの加工と画面への変換が分かれているので、どんな条件で絞っているかは `filter` の行だけ見れば把握できます。

## find と some — 1 件の取得と有無の判定

一覧の描画以外でも、配列のメソッドはあちこちで登場します。代表的な使い分けはこの 3 つです。
...
文言は、0 件の理由と次の行動（条件を変える）が伝わるものにします。0 件の画面は指示しないと後回しになりがちなので、「0 件のときの表示も作って」と一言添えると仕上がりが変わります。

## まとめ

- JSX の `{}` には式しか書けないので、一覧は文の for ではなく値になる `map`
- 一覧表示は「データの配列 → JSX の配列」への変換
- key は兄弟の中の名札で、同じデータなら毎回同じ値にする（id が最適）
