...
**選択肢 1: 全部揃うまで待つ**。サーバーでレビューまで取得してから HTML を返す。ページのどこよりも遅い処理に、**ページ全体が引きずられます**。商品名は 0.1 秒で出せるのに、ユーザーは 2 秒間白い画面を見ます。

**選択肢 2: 後からブラウザで取る**。ページはすぐ出して、レビューはブラウザで取得。今度は画面のあちこちで**スピナーがくるくる回り**、取得のための JavaScript も増えます。

「速い部分は今すぐ、遅い部分はできたときに、同じサーバー描画のまま」はできないのか。それを実現するのが **Suspense と Streaming** です。

## Suspense — 「準備中はこれを出しておいて」の境界
  ...
  return (
    <main>
      ...
      <Suspense fallback={<p aria-busy="true">レビューを読み込み中...</p>}>
        <Reviews productId={id} /> {/* 遅い（2 秒）。中で await している SC */}
      </Suspense>
    </main>
  );
}
...
この「少しずつ流す」配信が **Streaming**（ストリーミング）です。動画が全部ダウンロードされる前に再生が始まるのと同じ発想を、HTML に適用したものと言えます。

選択肢 1 と 2 の良いとこ取りになっていることを確認してください。

| | 全部待つ | クライアント取得 | Suspense + Streaming |
|---|---------|----------------|---------------------|
| 最初の表示 | 遅い | 速い | **速い** |
| 遅いデータの扱い | 全体を道連れ | ブラウザで取得（JS 増） | **サーバーが後から流す** |

## 境界をどこに引くか — 設計の話

Suspense の境界は技術の問題であると同時に、**「ユーザーに何を先に見せるか」という優先順位の宣言**です。
...
fallback の作りにも一工夫あります。ただの「読み込み中...」の文字より、**完成後と同じ高さ・形の置き物（スケルトン）**にすると、差し替え時に画面がガタッとずれる（レイアウトシフト）のを防げます。

## まとめ

- 「全部待つ」と「スピナー地獄」の間に、部品単位で待つ第三の道がある
