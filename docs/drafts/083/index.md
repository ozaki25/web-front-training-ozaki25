...
なぜ React は、たかが 1 部品のエラーでページ全体を道連れにするのでしょうか。

## 中途半端な画面より、何も無い画面

React の言い分はこうです。レンダリングの途中でエラーが起きたとき、**UI が中途半端な状態のまま画面に残るのは、消えるより危険**だからです。
...
- エラーバウンダリは延焼を止める防火壁。最初に出会った境界で止まる
- Next.js では error.tsx がフォルダ単位の防火壁。reset で復帰を試せる
- 守備範囲はレンダリング中だけ。イベントハンドラと非同期は別の手当てが要る
