...
AST を扱うツール群は今、大きな世代交代の途中です。

従来は **ESLint**（Lint）+ **Prettier**（整形）を組み合わせるのが定番でした。どちらも JavaScript で書かれていて、プラグインの生態系が豊富です。

最近は **Biome** のように、Lint と整形を 1 つのツールに統合し、Rust で高速に動かすアプローチが広がっています。**oxlint** も Rust 製の高速 Linter です。

| | ESLint + Prettier | Biome |
|---|---|---|
| 言語 | JavaScript | Rust |
| 速度 | 十分速いが大規模では重くなる | 桁違いに速い |
| 機能 | Lint と整形が別ツール | 1 つで両方 |
| プラグイン | 膨大な生態系 | まだ発展途上 |

どのツールを使っても、裏でやっていることは同じです。コードを AST にして、検査するか、変換するか、整形するか。道具の名前は変わっても、**AST という共通言語は変わりません**。
