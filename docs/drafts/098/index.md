...
これらはすべて、コードを**実行せずに**処理しています。なぜそれができるのか。答えは、全部が同じ入口から始まるからです。**コードを木にする**。
...
**実行せずに分かる**のは、すべてこの仕組みのおかげです。構造は完全に見えているから、「使われていない」「抜けている」「ここにメモ化が要る」という判断が機械的にできます。

一方、**実行しないと分からないこと**（この計算結果は正しいか、API は本当に動くか）は AST ベースのツールの守備範囲外です。構造の問題はツール、振る舞いの問題はテスト、という分担になっています。

エディタで「依存配列が抜けてますよ」と波線が出るのは、Lint が AST 上で「useEffect の関数内で参照している変数が、第 2 引数の配列にない」というパターンを検出しているからです。実行していないのに分かる、その裏側がこれです。
...
- 実行せずに構造の問題を検出できるのは、AST で意味の構造が見えているから
