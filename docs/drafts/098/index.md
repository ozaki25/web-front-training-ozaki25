...
今、この無駄を解消する動きが 2 つ同時に起きています。

**1 つは統合**。複数のツールが 1 つのパーサを共有し、1 回のパースで検査も整形もやる設計です。これは言語に関係なく、JavaScript で書いても実現できます。

**もう 1 つは高速化**。パーサ自体を Rust で書き直して、パースの速度を桁違いに上げるアプローチです。

この 2 つは独立した判断ですが、「どうせゼロから作り直すなら両方取ろう」と考えた結果、統合と高速化を同時に実現するツールが生まれています。

- **Biome**: Lint と整形を 1 つのツールに統合。Rust 製で高速。1 回のパースで検査と整形を両方やる
- **OXC**: Rust 製の高速パーサを核に、Linter（oxlint）・トランスパイラ・リゾルバを提供。バンドラの **Rolldown**（Vite の次期エンジン）もこのパーサを使う
