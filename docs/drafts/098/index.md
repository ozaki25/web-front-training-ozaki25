...
検査、変換、整形、削除。やることはバラバラなのに、どのツールもコードの**意味**を理解しているように見えます。文字列を正規表現で検索しているわけではないのに、「この変数は未使用」「この JSX は関数呼び出しに変わる」と判断できる。
...
この木があれば、ツールはコードを実行しなくても構造を理解できます。ここから先は、ツールによって「木をどう使うか」が違うだけです。
...
Lint は木のパターンを検査して問題を報告する。コンパイラは木のノードを別の形に書き換える。フォーマッタは木の構造を保ったまま表面だけ整える。バンドラは木をたどって到達できないコードを消す。

**どれも、コードを実行せずに処理している**のは同じです。AST があれば構造が完全に見えるので、実行しなくても「使われていない」「ここは変換できる」「この枝は不要」と判断できます。
