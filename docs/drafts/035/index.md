...
ブラウザで動くコンポーネントでは、これは不可能でした。レンダリングは「いま画面を描く」仕事で、何秒もかかる通信を待てないからです。Server Components でこれが許される理由は、実行の場がサーバーにあることで時間の使い方が根本的に変わるからです。

## サーバーでなら「待ってから描く」ができる

Server Components は**サーバーの中で実行され、描画結果だけがブラウザへ届きます**。
