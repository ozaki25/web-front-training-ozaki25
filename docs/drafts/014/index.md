...
::: info 氏名・住所は「普通の日本語だけ」ではない
人名や地名には「𠮷」（つちよし。𠮷野家の𠮷）のように、番号が 65535 を超えてサロゲートペアになる漢字が混じることがあります。氏名・住所のフォームを「常用漢字しか来ない」と決めつけないでください。

なお「﨑」「髙」のような旧字体は番号が 65535 以下なので箱 1 つ、`.length` はズレません。同じ「旧字体」でもズレるもの（𠮷）とズレないもの（﨑）があります。「旧字体だから危険」ではなく「番号が 65535 を超える文字だけが危険」です。
:::
...
**「自由入力かどうか」がポイント**です。氏名・住所・コメント・プロフィールのような自由入力は、絵文字や「𠮷」のようなサロゲートペアの漢字が来うるので対応が必要です。一方、数字や選択肢しか入らない形式固定のフィールドは気にしなくて OK です。「住所は普通の日本語だから安全」という思い込みが、まれな漢字や絵文字で足をすくいます。
...
- 絵文字や一部の漢字（𠮷 などサロゲートペア。氏名・住所にも混じる）は箱 2 つ以上になり、`.length` がズレる
- 切り取り（`slice`）もズレる。配列にしてから切ると安全
- 見た目どおり数えるなら `Intl.Segmenter`（書記素単位）
- 自由入力フィールド（氏名・住所・コメント）は対応必須、形式固定の入力は不要。AI に「絵文字やサロゲートペアを考慮して」と伝える
