...
「𠮷」（つちよし。𠮷野家の𠮷）や「﨑」「髙」のように、人名・地名・店名には旧字体・異体字（サロゲートペアの漢字）がよく使われます。住所も「普通の日本語だから安全」とは言えません。氏名・住所のフォームを「常用漢字だけ来る」と決めつけないでください。
:::
...
**「自由入力かどうか」がポイント**です。氏名・住所・コメント・プロフィールのような自由入力は、絵文字も旧字体（「﨑」「髙」「𠮷」など）も来うるので対応が必要です。一方、数字や選択肢しか入らない形式固定のフィールドは気にしなくて OK です。「住所は普通の日本語だから安全」という思い込みが、地名や建物名の旧字体で足をすくいます。
...
- 絵文字や一部の漢字（サロゲートペア。氏名・住所の旧字体も）は箱 2 つ以上になり、`.length` がズレる
