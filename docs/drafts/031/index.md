...
もう 1 つは `useQuery` のようなライブラリ派です。なぜわざわざライブラリを使うのか。手書き版は一見動くのに、**穴だらけ**だからです。
...
### 穴 4: 同じデータを複数の場所で取ると、リクエストも複数飛ぶ
...
キャッシュには「古くなる」問題が付きものです。ライブラリの戦略は秀逸で、**キャッシュをまず即座に表示し、裏で取得し直して、変わっていたら差し替える**というものです。ユーザーは一瞬も待たされず、データは勝手に新しくなります。

この戦略の名前が stale-while-revalidate（古いものを出しつつ再検証する）。もう 1 つの定番ライブラリ **SWR** の名前は、まさにこの頭文字です。両者は思想がほぼ同じで、TanStack Query が多機能、SWR が軽量という住み分けです。
