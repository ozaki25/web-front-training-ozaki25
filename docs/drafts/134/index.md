...
> **サードパーティスクリプト**: 第三者である他社が配信し、自分のページに読み込んで実行するスクリプト

## 自分のコードと同じ権限で動く外部スクリプト
...
> **XSS**: コメント欄などに書かれたユーザーの入力が、エスケープ漏れによって HTML やコードとして解釈されてしまう攻撃
...
> script タグで信頼しているのは、今この瞬間のファイルではなく、**その URL が今後返すものすべて**
...
Subresource Integrity、略して **SRI** は、読み込むファイルの中身をブラウザに検証させる Web 標準です。script タグに `integrity` 属性で「期待するファイルのハッシュ値」を書いておきます。
...
**CSP** という HTTP レスポンスヘッダー（Content-Security-Policy）を使うと、「許可した提供元以外のスクリプトは読み込まない」とブラウザに宣言できます。
...
「解析タグを埋め込んで」と AI に頼んで、タグを 1 行貼る作業自体は任せて構いません。ただ、その提供元を信頼してよいか、決済ページにまで入れる必要があるかは、貼る前に人が判断することです。
