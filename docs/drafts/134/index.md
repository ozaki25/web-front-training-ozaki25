...
ただしこの 1 行は、機能を有効にするスイッチではありません。**他社のサーバーに置いてある JavaScript を、自分のページの中で実行する**という指示です。こうして外部から読み込むスクリプトを**サードパーティスクリプト**（第三者が配信するスクリプト）と呼びます。今日の話は、この「他社のコードを自分のページで動かす」ことが何を意味するかです。

## 外部スクリプト用の隔離領域はない

ブラウザは、`<script src="...">` で読み込んだコードを、読み込み元がどこのサーバーであっても同じように扱います。自分で書いたスクリプトも、他社から読み込んだタグも、**同じ DOM、同じグローバルスコープ**で動きます。「外部のスクリプトだけ別の部屋で動かす」という仕組みはありません。
...
「他人のコードが自分のサイトで動く」と聞くと、XSS を思い浮かべるかもしれません。XSS は、コメント欄などに書かれた**ユーザーの入力**が、エスケープ漏れによって HTML やコードとして解釈されてしまう攻撃です。今日の話はそれとは別物です。
...
XSS は「入れるつもりのなかったコード」が紛れ込む事故で、自分のコードを直せば防げます。サードパーティスクリプトは**自分から招き入れたコード**なので、エスケープでは防げません。信頼するかどうかを決めた時点で、その後の中身の変化ごと引き受けています。
...
どの場合も、**サイト運営者は何も変えていないのに**、次にページが開かれた瞬間から改ざんされたコードが動き始めます。script タグは「今この瞬間のファイル」ではなく「その URL が今後返すものすべて」を信頼する行為だからです。
...
タグは連鎖するのも厄介な点です。広告タグが入札のために別のタグを読み込み、それがまた別のスクリプトを読み込む。**入れるスクリプトが 1 つ増えるたびに、信頼しなければならない相手の連鎖が伸びていきます**。
...
最初の対策は技術ではなく判断です。実績のある提供元だけを選ぶこと、そして入れるタグの数を必要最小限に絞ることです。特に決済ページや個人情報の入力ページには、動作に不要なタグを置かないのが定石です。
...
ハッシュ値はファイルの中身から計算される指紋のような値で、中身が 1 バイトでも変われば別の値になります。ブラウザはダウンロードしたファイルのハッシュを計算し、`integrity` の値と一致しなければ**実行を拒否**します。提供元でファイルが改ざんされても、改ざん後のコードは動きません。

ハッシュには sha256、sha384、sha512 が使えます。別ドメインのファイルを検証するには `crossorigin` 属性もあわせて必要です。

ただし SRI には向き不向きがあります。ハッシュは特定の中身に対する指紋なので、**提供元が中身を頻繁に更新するスクリプトには使えません**。多くの解析・広告タグは提供元が随時中身を更新する方式なので SRI を付けられず、バージョンを固定して配信されるライブラリに向いた仕組みです。
...
悪意のあるスクリプトがページに紛れ込んでも、HttpOnly のセッション Cookie は盗めません。改ざんを完全には防げない前提で、盗まれて一番痛いものを金庫に入れておく発想です。1 つの対策で全部を守るのではなく、破られたときの被害を次の層で小さくする。こういう重ね方を**多層防御**と呼びます。
