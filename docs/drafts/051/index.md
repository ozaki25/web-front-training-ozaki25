...
これらの属性はどれも、この「自動送信をどう絞るか」を決めるものです。実際の `Set-Cookie` では、名前と値のうしろに `;` で並べて書きます。

```
Set-Cookie: session_id=abc123; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=3600
```

属性には2種類あります。`HttpOnly` と `Secure` は値を取らないフラグで、その語を書けば有効、書かなければ無効です。

`SameSite` や `Path`、`Max-Age` は `=` のうしろに値を書きます（`Max-Age` は秒数、`SameSite` は `Lax`/`Strict`/`None` のいずれか、という具合です）。
...
`Path` は同じホストの中の URL の範囲です。

- `Path=/`（ふつう）: そのホストの全 URL に送る
- `Path=/admin`: `/admin` 以下にだけ送り、`/` や `/shop` には送らない

ただし Path はセキュリティの壁にはなりません。同一オリジンなら別のパスから読み出せてしまうので、範囲を軽く絞る程度に考え、ふつうは `/` のままにします。
