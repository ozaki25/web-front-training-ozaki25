...
- その隙が CSRF を生み、SameSite や HttpOnly で送り方を絞ると知る
...
だから `example.com` の Cookie は `example.com` 宛てにしか乗らず、`attacker.com` に渡ることはありません。ここは安心してよい部分です。

ただし、抜けがあります。ブラウザは長い間、**そのリクエストがどのページから発せられたか**を見ていませんでした。

だから `attacker.com` を開いているときでも、そこから `example.com` へ送るリクエストには、宛先が `example.com` である以上、`example.com` の Cookie が付いてしまいます。

この「宛先は見るが、発信元は問わない」という隙が、次に見る CSRF を生みます。

## 自動送信が生む危険と属性での制御

`example.com` にログイン中のまま別のサイトを開くと、そのサイトが仕込んだ `example.com` への送信にも、ブラウザは Cookie を付けてしまいます。利用者は操作した覚えがないのに、ログイン済みの操作が成立してしまいます。

これが、別サイトからあなたになりすます CSRF という攻撃の入り口です。

抑えるのが `SameSite` 属性です。送る前に「発信元が同じサイトか」を見て、別サイト発の送信には Cookie を付けないようにします。

主要ブラウザは、指定のない Cookie を `Lax` として扱います。

`SameSite` が見る「同じサイト」は、よく same-origin と混同されます。基準を整理します。
...
`SameSite` が見るのは same-site のほうです。一方、JavaScript が別サイトのデータを読めるかは、より厳密な same-origin で判定されます。
...
ここまでの属性を含め、実際の `Set-Cookie` では名前と値のうしろに `;` で並べて書きます。
...
送る範囲を決める `SameSite` と `Domain` は、既定が狭い側に寄っています。広げるのは必要なときだけ、と考えるのが安全です。
