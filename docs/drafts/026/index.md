...
以前からある `useContext(UserContext)` も同じ用途の API で、既存のコードでは今もよく見かけます。`use()` は `if` 文の中でも呼べるなど制約が少ないので、新しく書くならこちらでそろえられます。

::: tip use() は Context 専用ではない
`use()` には Promise も渡せて、データ取得の結果を待つ用途にも使われます。今日は Context を読む使い方だけ覚えれば十分です。
:::
...
Context の値が変わると、**その Context を `use()` で読んでいるコンポーネントすべてが再レンダリングされます**。途中のコンポーネントを飛ばして、**読んでいる全員に直接波及します**。

```mermaid
flowchart TD
  A["UserContext の value が変わる"] --> B["use(Ctx) している<br>コンポーネント A → 再レンダリング"]
...
props なら「この props を渡している親子だけ」と影響範囲を追えますが、Context はツリーのどこで読まれているかが**コードからは見えにくくなります**。入力のたびに変わるような値を乗せると、自分では気づかないうちに画面のあちこちが連鎖的に再描画されて重くなります。

**変更頻度が低ければ、全員への波及もまれに抑えられます**。「たまにしか変わらないもの」という判断基準は、ここから来ています。

また、Context に乗せたデータは、誰がどこで使っているかが props のように追えません。何でも乗せると、データの流れが見えないアプリになります。

**まず props、本当に広く共有するものだけ Context** が基本姿勢です。Context 化の提案を受けたときも、「全体で共有すべき値か」「変更頻度は低いか」の 2 点で判定できれば、鵜呑みにせずに済みます。

## まとめ

- 使わない props を運ぶだけの中継がバケツリレー（prop drilling）
- Context は上に置いた値を中継なしで深くから読む仕組みで、`<Ctx value>` で提供し `use(Ctx)` で読む
- 値が変わると読んでいる全員が再レンダリングされる
- 基本は props、全体共有で低頻度変更のものだけ Context
