...
- JavaScript はダウンロード + パース + 実行の 3 段階。サイズが直接速度に効く
- tree shaking は不要コードの除去、code splitting はページ単位の分割。どちらも自動
