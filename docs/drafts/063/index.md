...
```mermaid
flowchart TB
    S1[親の中で普通に流れている<br/>相対配置と同じ扱い] --> S2{top の位置に<br/>到達した?}
    S2 -- まだ --> S1
    S2 -- 到達 --> S3[ビューポートに張り付く<br/>fixed のように振る舞う]
    S3 --> S4{親の下端を<br/>超えた?}
    S4 -- まだ --> S3
    S4 -- 超えた --> S5[親と一緒にスクロールアウト]
```

| プロパティ | 基準 | 固定期間 | 典型用途 |
| --- | --- | --- | --- |
| `fixed` | ビューポート | 常時 | 画面右下のチャットボタン、モーダル |
| `sticky` | 最も近いスクロールコンテナ | 親の範囲内だけ | セクション見出し、ページ内ヘッダー |
...
「着地点そのものをずらす」発想で、JS なしで直ります。全要素に付けるとレイアウトに影響する可能性があるので、アンカーの着地点になる見出しなどにスコープを絞るのが安全です。

### 他にも覚えておくと効く小技

```css
/* 内側スクロール領域で親ページまで動く「スクロール貫通」を止める */
.modal-body {
  overflow: auto;
  overscroll-behavior: contain;
}

/* スクロールバーが出たり消えたりでレイアウトが 1〜2px ずれるのを防ぐ */
html {
  scrollbar-gutter: stable;
}
```

- `overscroll-behavior: contain` — モーダルや内側スクロール領域の端まで来ても、背後のページが動かない
- `scrollbar-gutter: stable` — スクロールバー分の幅を常に確保しておくことで、コンテンツ量が変わってもレイアウトが安定する
- `scroll-driven animations`（`animation-timeline: scroll()`）— スクロール量に連動するアニメーション。Chrome/Edge では 2024 年から安定利用でき、Safari 26・Firefox（2026-04 時点）でも段階的に実装が進行中。使うときは従来の CSS トランジションで見栄えが壊れない「プログレッシブエンハンスメント」として組み込むのが安全です
