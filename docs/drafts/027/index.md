...
Redux であれほど書いたものが、ファイル 1 つに収まります。さらに Context と違って「**使っている値が変わったコンポーネントだけ再レンダリング**」が標準なので、性能面の落とし穴も塞がれています。現在の新規プロジェクトでグローバル状態が必要なら、この世代が第一候補です。
...
- サーバーのデータは状態でなくキャッシュ。専用ライブラリかサーバー側へ
