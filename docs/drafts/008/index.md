...
「状態が変わるたびに画面を作り直す」と聞くと、毎回画面全体を描き直して遅いのでは、と思うかもしれません。実際にそうなると、ちらつきや遅延が起きます。

React はこれを避けるために<strong>仮想 DOM</strong>という仕組みを使います。画面の構造を JavaScript のデータとしてメモリ上に持っておき、状態が変わると「新しい画面の構造」と「前の構造」を比較します。そして<strong>実際に変わった部分だけ</strong>を本物の DOM に反映します。

```mermaid
flowchart LR
  State["状態が変わる"] --> New["新しい画面構造を作る"]
  New --> Diff["前の構造と比較<br>（差分を計算）"]
  Diff --> Patch["変わった部分だけ<br>DOM を書き換える"]
```

カウンターの数字だけが変わったなら、書き換わるのはその数字だけです。開発者は「作り直す」と書いているつもりでも、裏では最小限の更新だけが行われています。「全部宣言し直す書きやすさ」と「最小限だけ更新する速さ」を両立させているのが、この仕組みです。
...
- React は<strong>仮想 DOM</strong>で前後の差分を計算し、実際に変わった部分だけを更新する。だから「作り直す」書き方でも速い
