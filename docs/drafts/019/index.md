...
`<main>` や `<h1>` は HTML のタグですが、`<OrderCard />` は HTML のどこを探してもありません。これは**コンポーネント**、つまり自分（または AI）が定義した部品です。

タグのような見た目なのに、`title="コーヒー豆"` のように渡す値を変えると表示が変わります。
...
ただの**関数**で、特別な登録も継承もありません。React のコンポーネントとは「JSX（画面の部品の記述）を返す関数」のことです。
...
`<main>` は HTML タグとして、`<OrderCard>` は関数として扱われます。React はこの区別を**頭文字が大文字かどうか**で行います。
...
データは親から子へ一方通行で流れ、子は受け取って表示するだけです。この一方通行のおかげで、表示がおかしいとき「どこから渡された値のせいか」を親へ親へとたどるだけで原因を探せます。

値を変えたい場合は、`const taxed = price * 1.1;` のように加工した別の変数を作ります。
...
`Panel` は「枠」だけを定義し、中身は使う側が自由に決められます。枠と中身を分業できるので、カード・モーダル・レイアウトなど「囲むもの」はほぼこの形で書かれます。
...
`&&` は左側が真のときだけ右側を返します。React のコードでは「片方しか無い三項演算子」としてよく使われます。
...
`&&` の左側に数値を置くと、思わぬ表示が起きます。

```tsx
{cartCount && <p>カートに {cartCount} 点あります</p>}
```

`cartCount` が `0` のとき、何も表示されないのではなく**画面に `0` と表示されます**。`&&` は左が偽ならその左の値を返す仕組みで、JSX は `false` は描画しませんが **`0` は数値として描画する**からです。
...
この書き方は AI のコードにも紛れ込みます。画面の隅にポツンと `0` が出ていたら「`&&` の左が数値になっていないか」を疑うと、原因を言葉にできて修正の指示も一言で済みます。

## まとめ

- コンポーネントは JSX を返す関数で、大文字始まりで HTML タグと区別される
- props はタグの属性に見える読み取り専用の引数で、親から子への一方通行
- タグで挟んだ中身は `children` として渡される
- 出し分けは三項演算子・`&&`・early return で、`&&` の左が `0` だと画面に出る
