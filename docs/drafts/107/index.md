...
自分が何か更新したわけではないのに、画面のデータが古くなっていることがあります。たとえば別の管理画面で在庫が変わった、他の人が情報を更新した、といった場合です。こういうときは、クライアントコンポーネントから `router.refresh()` を呼ぶと、今の画面を最新の状態で取り直せます。
...
`router.refresh()` は、今いるルートの保持データを消してサーバーから取り直します。入力中のフォームの状態などは保ったまま、表示するデータだけを新しくできます。

`revalidatePath` がサーバー側のキャッシュを「古い」と印つけて全ユーザーに効くのに対し、`router.refresh()` はサーバー側のキャッシュには触れず、自分のブラウザで今の画面を取り直すだけです。「データを変えた側がキャッシュを消す」のが `revalidatePath`、「変えていない側が取り直す」のが `router.refresh()` です。
