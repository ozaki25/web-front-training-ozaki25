...
- `:has()` は「〜を持つ親」にスタイルを当て、子の状態で親が変わる
- コンテナクエリは画面幅でなく親の幅でスタイルを変え、配置場所に自動適応する
- CSS ネストは素の CSS でセレクタの入れ子が書けて、構造が見やすくなる
- JavaScript でやっていた制御が CSS に移る流れで、CSS で済むなら速くて軽い
