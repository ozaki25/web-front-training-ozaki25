...
「投稿しました」は嘘ではありません。プライマリには確かに書けています。ただ、読んだ相手が複製の追いついていないレプリカだった。リロードで直るのは、そのわずかな間に複製が追いつくからです。

## 結果整合性というトレードオフ

すべてのコピーを常に一致させることもできます。書き込みのたびに、全レプリカへの反映が終わるまで待って「成功」を返せばいい。ただしその分だけ書き込みは遅くなり、レプリカが 1 台でも不調だと書き込み自体が止まります。

そこで多くのサービスは逆の割り切りをします。コピーが常に一致することは諦めて、速さと止まりにくさを取る。書いた内容はいずれ全コピーに行き渡るけれど、一瞬はずれていてもよい、とする考え方です。これを**結果整合性**（eventual consistency）と呼びます。
