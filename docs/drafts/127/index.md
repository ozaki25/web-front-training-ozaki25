...
SNS にコメントを書き込んで、「投稿しました」と表示された。ところが一覧に戻ると、自分のコメントがどこにもない。不安になってもう一度投稿しかけたけれど、数秒待ってリロードしたら、ちゃんと出てきた。

こういう体験は、大きなサービスでもたまに起きます。開発中のアプリで起きてバグとして報告すると、「それは仕様です」と返ってくることさえあります。

保存は本当に成功しています。それなのに、直後の読み取りには反映されていない。この一見矛盾した動きは、データの置き場所が 1 か所ではないことから生まれます。
...
この複製は一瞬では終わりません。普段は数ミリ秒でも、負荷が高いときには数秒遅れることがあります。この遅れを**レプリケーションラグ**と呼びます。
...
「投稿しました」は嘘ではありません。プライマリには確かに書けています。ただ、読みに行った先が、複製の追いついていないレプリカだっただけです。リロードで直るのは、そのわずかな間に複製が追いつくからです。

## 結果整合性というトレードオフ

すべてのコピーを常に一致させることもできます。書き込みのたびに、全レプリカへの反映が終わるまで待って「成功」を返せば済みます。ただしその分だけ書き込みは遅くなり、レプリカが 1 台でも不調だと書き込み自体が止まります。

そこで多くのサービスは逆に割り切って、コピーが常に一致することは諦め、速さと止まりにくさを取ります。書いた内容はいずれ全コピーに行き渡るけれど、一瞬はずれていてもよい、とする考え方です。これを**結果整合性**（eventual consistency）と呼びます。
...
結果整合性で困るのは、実はほぼ 1 場面だけです。他人の投稿が数秒遅れて見えても誰も気づきません。でも**自分の投稿が見えないのは、本人にはバグにしか見えません**。

この「自分が書いたものは、自分にはすぐ見えてほしい」という要求には **read-your-own-writes**（自分の書き込みは自分で読める）という名前が付いています。全体は結果整合性のままでよく、自分の分だけ保証すればいい、という絞り込みです。
...
サーバーの応答を待たずに画面を先に変える楽観的更新という手法もありますが、あちらは成功を仮定して見せる話です。ここでのやり方はサーバーが確定した結果を使うので、成功は確認済みです。ずれているのは読み直し先のデータだけなので、応答を使えば正しい表示になります。
...
Next.js にも、この用途の API があります。一覧をタグ付きでキャッシュしている場合、Server Action の中で `updateTag(tag)` を呼ぶと、そのタグのキャッシュが即時に無効化され、自分の書き込みが直後の表示に反映されます。read-your-own-writes のための即時無効化として用意されていて、Server Action 専用です。
...
この構造を知っていると、あやふやなバグ報告に当たりが付けられます。「リロードしたら直った」「たまに出ないことがある」「手元では再現しない」。この 3 つがそろったら、結果整合性を疑う価値があります。
...
チームの会話でも「これは結果整合性では」「read-your-own-writes はどう保証していますか」と聞けると、話が一気に具体的になります。AI に実装を指示するときも「作成 API の応答を一覧に直接反映して」「保存後は updateTag でキャッシュを無効化して」と、欲しい挙動を一言で伝えられます。
