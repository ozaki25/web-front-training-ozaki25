...
スマホの文字をかなり大きくして使っている人は、身近にいるはずです。家族のスマホをのぞいたら文字が巨大だった、細かい文字のページを指でつまんで広げた、といった経験は多くの人にあるでしょう。

ところが、拡大した途端に崩れるサイトがあります。

- 横にスクロールしないと、文の続きが読めない
- ボタンが重なって押せない
- 文字の後半が切れて見えない

弱視の人や高齢の人にとって、拡大は「あれば便利」ではなく「ないと読めない」機能です。

> 拡大に耐えない画面は、その人たちには読めない画面

今日は、なぜ壊れるのか、どう作れば壊れないのかを仕組みから見ます。
...
ページズームで 200% にすると、画面に横に並べられる量は実質半分になります。

- 幅 1200px のディスプレイで見ていても、レイアウト上は 600px の画面で見ているのと同じ
- つまり「拡大に耐える」は「狭い画面に耐える」とほぼ同じ問題

文字サイズ変更のほうは、画像や余白はそのままで文字だけが大きくなります。

- 文字を入れている箱が一緒に広がらないと、あふれたり切れたりする
- こちらの対策は、あとで出てくる `rem` が担当する

## リフローという考え方

画面幅に対して中身が多すぎるとき、要素を折り返して縦に並べ直すことを**リフロー**と呼びます。

紙の新聞は幅が固定なので、拡大コピーすると紙からはみ出します。うまく作られた Web ページは違います。

- 幅が足りなくなったら、3 列を 2 列に、2 列を 1 列に組み替える
- **縦スクロールだけで読み進められる形**に流れ直す

これがリフローです。200% に拡大したときの読み方が、リフローの有無で変わります。

| リフロー | 200% に拡大したときの読み方 |
|---------|---------------------------|
| 働いている | 上から下へスクロールするだけ。読み方は変わらない |
| 壊れている | 1 行読むごとに右へスクロールして左へ戻る操作を強いられる |

Web アクセシビリティのガイドラインである WCAG（Web Content Accessibility Guidelines）にも、対応する基準があります。

- **1.4.4 テキストのサイズ変更**: 文字を 200% まで拡大しても、内容や機能が失われないこと
- **1.4.10 リフロー**: 狭い画面幅でも、一方向のスクロールだけで読めること

数値の細部より、「200% で壊れないか」という確かめ方を覚えておけば十分です。
...
`width: 800px` の要素は、画面が 600px 相当になっても 800px のままです。

- 幅を譲らないので、画面からはみ出すか、隣の要素に重なる
- ページのどこか 1 か所にこれがあるだけで、ページ全体に横スクロールが発生する
...
- `1rem` は、ルート要素（`html`）の文字サイズを基準にした単位
- 多くの環境で初期値は 16px、そして**利用者がブラウザ設定で変えられる**
- 幅や余白を `rem` で書いておくと、利用者が文字を大きくしたとき、文字を入れる箱も同じ比率で広がる
...
ひとつ注意があります。`html { font-size: 16px; }` のように基準値そのものを px で固定すると、利用者のフォント設定が効かなくなります。

> `rem` の土台であるルートの文字サイズは、px で上書きしない
...
かつては「ネイティブアプリらしい見た目を保つ」目的で広く使われた指定です。しかし拡大を必要とする人から、その手段ごと奪ってしまいます。

- 最近はこの指定を無視するブラウザもあるが、無視される前提で残してよいものではない
- でき上がったコードにこの指定を見つけたら、外す
