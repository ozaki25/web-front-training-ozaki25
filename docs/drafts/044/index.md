...
- フォーカスはキー入力の現在地で、Tab で DOM 順に巡回する
- `outline: none` は現在地の喪失で、消すなら `:focus-visible` で出し分ける
- `tabindex` は 0 と -1 だけ使い、正の値は禁じ手
- モーダルは「開いたら中へ・外に出さない・閉じたら元へ」で、`<dialog>` なら大半が自動
