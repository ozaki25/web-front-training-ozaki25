# フォームバリデーションの層 — どこで、何を、何回検証するか

## 今日のゴール

- 検証には 3 つの層（HTML・クライアント JS・サーバー）があると知る
...
```mermaid
flowchart LR
  A["層 1: HTML<br>required, type=email"] --> B["層 2: クライアント JS<br>入力中の細かい検証"]
  B --> C["層 3: サーバー<br>最終防衛ライン"]
...
ただし、ここで絶対に忘れてはいけない事実があります。**層 1 と層 2 は、どちらも簡単に突破できます**。ブラウザの開発者ツールで `required` を消すのも、JavaScript を無効にするのも、フォームを通さず直接リクエストを送るのも自由です。画面側の検証は**善意のユーザーの入力ミスを早く優しく教えるため**のもので、防御ではありません。
...
import { signupSchema } from "./schema"; // 層 2 と同じスキーマを使う
...
ここで効いてくるのが、**層 2 と層 3 で同じスキーマを import している**ことです。検証ルールを画面用とサーバー用に二重に書くと、片方だけ直して食い違う事故が必ず起きます。**ルールは 1 か所（スキーマ）に書き、両方の層から使う**。これが zod のようなスキーマライブラリが支持される最大の理由です。
