...
- パスワードの保存は bcrypt など専用のライブラリに任せると知る

## 入力したパスワードの行き先

会員登録のフォームにパスワードを入力して送信すると、パスワードはサーバーに届き、データベースに保存されます。次のログインで照合するためです。

問題は、**どんな形で保存するか**です。ここを間違えると、データベースが漏洩したときに全利用者のパスワードが流出します。AI にログイン機能を任せるときも、この保存方法の判断は指示する側が持っておくべき土台です。
...
「うちのデータベースは漏れない」とは誰にも言い切れません。大手のサービスでも漏洩事故は繰り返し起きています。なので「漏らさない」だけに頼るのではなく、**漏れても被害が広がらない形で保存しておく**のが定石です。その形がハッシュ化です。

## ハッシュ化という一方向の変換

**ハッシュ関数**は、入力の文字列から決まった手順で別の文字列を計算する関数です。計算結果を**ハッシュ値**と呼びます。たとえば SHA-256 というハッシュ関数に通すと、こうなります。
...
同じ入力からは、必ず同じハッシュ値が計算されます。入力が1文字でも違えば、全く別の値になります。そして、ハッシュ値から元の文字列を計算で逆算することはできません。この後戻りできない性質を**一方向性**と呼びます。

この性質を使うと、パスワードそのものを保存する必要がなくなります。登録時にパスワードをハッシュ化して、**ハッシュ値だけを保存する**のです。データベースが漏れても、攻撃者の手に入るのはハッシュ値だけで、元のパスワードは分かりません。
...
人が決めるパスワードには偏りがあります。`password123` や `123456` のようなよくあるパスワードは、数がたかが知れています。そこで攻撃者は、よくあるパスワードを大量に集め、それぞれのハッシュ値を**事前に**計算した対応表を用意しておきます。この対応表は**レインボーテーブル**と呼ばれます。

データベースが漏れたら、あとは盗んだハッシュ値を対応表から引くだけです。逆算はできなくても、「答え合わせ」は一瞬でできてしまいます。
...
同じ `password123` でも、ソルトが違えばハッシュ値は全く別になります。攻撃者が事前に作った対応表は「ソルトなしのハッシュ値」の表なので、役に立たなくなります。ソルト入りで作り直そうにも、ソルトは利用者ごとに違うため、1人分の表を作っても他の利用者には使えません。

意外に思えるのが、**ソルトは秘密にしなくてよい**という点です。ソルトはハッシュ値と一緒にデータベースへ保存します。ソルトの目的は「隠すこと」ではなく「事前計算を無効にすること」だからです。ソルトが攻撃者に見えても、利用者1人ずつ総当たりをやり直すしかない状況は変わらず、事前の一括計算という近道は消えたままです。
...
SHA-256 のような汎用のハッシュ関数は、大量のデータを高速に処理するために作られています。この速さが、パスワード保存では逆に弱点になります。速く計算できるほど、攻撃者が候補を試す速度も上がるからです。GPU を使えば1秒間に数十億回の計算も可能とされ、短く単純なパスワードなら現実的な時間で割り出せてしまいます。
...
大事なのは、**この処理を自分で組み立てないこと**です。ソルトの生成やつなげ方、比較のしかたには細かい注意点が多く、自作の実装は事故のもとです。実績のあるライブラリに任せるのが現実的な結論です。
