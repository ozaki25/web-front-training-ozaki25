...
`--blue-500` は「この色は何色か」を表します。`--color-primary` は「この色は何のために使うか」を表します。

使う側は `--color-primary` を参照します。ブランドカラーが青から緑に変わっても、`--color-primary` の定義元を差し替えるだけで、使っている側のコードは一切触りません。
